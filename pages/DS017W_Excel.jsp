<%
//98.04.21 create A08 存款帳戶分級差異化管理統計資料 by 2295
...
<%
	...
	try{
			...
			prop_column.load(new FileInputStream(Utility.getProperties("schemaDir")+System.getProperty("file.separator")+"A08_ALL_detail.TXT"));
			//=======================================================================================================================
			...
			prop_column_name.load(new FileInputStream(Utility.getProperties("schemaDir")+System.getProperty("file.separator")+"A08_ALL_column.TXT"));
			//====================================================================================================
            ...
            (
              ...
              left join (select * from A08
              	   		 where (to_char(m_year * 100 + m_month) >= 9501 and to_char(m_year * 100 + m_month) <= 9810) 
            			 and bank_code in ('6060019','6030016')
              	   		)A08 on bn01.bank_no = A08.bank_code
              where A08.bank_code  <>  ' ' 
            ...
            select  cd01.FR001W_output_order as FR001W_output_order,
            		...
            		m_year || '/' || decode(substr(to_char(m_month),2),'','0','') || decode(substr(to_char(m_month),1),'1','1',substr(to_char(m_month),1)) as m_yearmonth,
            		m_year,m_month,       	
            		sum(A08.warnaccount_cnt)  as warnaccount_cnt,           		
            		sum(A08.limitaccount_cnt) as limitaccount_cnt,            		
            		sum(A08.erroraccount_cnt)  as erroraccount_cnt,                    
                	sum(A08.otheraccount_cnt)  as otheraccount_cnt,
             		sum(A08.depositaccount_tcnt )  as depositaccount_tcnt 
            from  (select * from cd01 where cd01.hsien_id <> 'Y') cd01
            left join wlx01 on wlx01.hsien_id=cd01.hsien_id
            left join bn01 on wlx01.bank_no=bn01.bank_no  and bn01.bank_type='6'
            left join (select * from A08 
            ...
            */            
            ...
            StringBuffer  a08_table= new StringBuffer() ;//a08
            List a08List = new ArrayList() ;
            a08_table.append(" (select * from ").append(cd01Table).append(" cd01 where cd01.hsien_id <> ?) cd01"
            ...
            a08_table.append("ALL".equals(bank_type) ? "? ,?" : " ?" );
            a08_table.append(")"
              	   				  + " left join (select * from A08 "
            ...
            if("ALL".equals(bank_type) ) {
            	...
            	a08List.add(bank_type) ;
            }
            a08List.add(S_YEAR+S_MONTH) ;
            a08List.add(E_YEAR+E_MONTH) ;
			if(hasBankListALL.equals("false")){	   				  
			   a08_table.append("         and  bank_code in ("+selectBank_no+")");
			}	   				  
            a08_table.append("           )A08 on bn01.bank_no = A08.bank_code");
            ...
            sqlCmd.append(" select bank_no,bank_name,"
            	   ...
            	   + " ) A08 ");
				   //+ " order by fr001w_output_order,bank_no,m_year,m_month";
            for(int a=0 ;a<a08List.size();a++) {
            	sqlCmdList.add(a08List.get(a));
            }
            a08_field_sum = " sum(A08.warnaccount_cnt)  as warnaccount_cnt,"            		  
            		   	  + " Sum(A08.limitaccount_cnt) as limitaccount_cnt,"            		 
            		   	  + " Sum(A08.erroraccount_cnt)  as erroraccount_cnt,"                       				
            	       	  + " Sum(A08.otheraccount_cnt)  as otheraccount_cnt,"
              		   	  + " Sum(A08.depositaccount_tcnt)  as depositaccount_tcnt";
			...
			for(int a=0 ;a<a08List.size();a++) {
				sqlCmd_sumList.add(a08List.get(a));
            }
			if(!order.equals("")){
			    //各別機構
			...
			p.load(new FileInputStream(Utility.getProperties("schemaDir")+System.getProperty("file.separator")+"A08_ALL_column.TXT"));
			//====================================================================================================
            ...
            wb.setRepeatingRowsAndColumns(0, 1, columnLength+3, 1, 7); //設定表頭 為固定 先設欄的起始再設列的起始
  			...
  			//System.out.println("DS017W_Excel.sqlCmd="+sqlCmd);
  			//System.out.println("ori_field="+ori_field);
  			List dbData = null;
  			if(hasBankListALL.equals("false")){
			  dbData = DBManager.QueryDB_SQLParam(sqlCmd.toString(),sqlCmdList,"warnaccount_cnt,limitaccount_cnt,erroraccount_cnt,otheraccount_cnt,depositaccount_tcnt");
			  //System.out.println("DS017W_Excel.sqlCmd="+sqlCmd);
			}else{//95.09.04 add 金融機構代號=ALL全部
			  dbData = DBManager.QueryDB_SQLParam(sqlCmd_sum.toString(),sqlCmd_sumList,"warnaccount_cnt,limitaccount_cnt,erroraccount_cnt,otheraccount_cnt,depositaccount_tcnt");
			  //System.out.println("DS017W_Excel.sqlCmd_sum="+sqlCmd_sum);
			}
