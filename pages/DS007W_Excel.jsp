...
// 95.10.25 農.漁會.都使用同一個設定檔 by 2295
// 95.10.27 add 縣市別小計農漁會 by 2295
...
<%
	...
	try{
		...
		wlx01.append(" HSIEN_DIV_1,CANCEL_NO,CANCEL_DATE from wlx01 where m_year =? )");
			...
			prop_column.load(new FileInputStream(Utility.getProperties("schemaDir")+System.getProperty("file.separator")+"F01_6_detail.TXT"));			
			//=======================================================================================================================			
			...
			String f01_operation_field_sum="";
			String f01_operation_field="";
			String f01_field_sum="";			
			String ori_field="";
			String f01acc_code="";
			String f01_field_sum_hsien_id = "";//縣市別用	
			String f01_field_hsien_id = "";//縣市別用	
			        ...
			        if(detail_column != null && detail_column.size() != 0){               
			           columnLength += detail_column.size();//累加總欄位個數
              		   for(j=0;j<detail_column.size();j++){
              		   	   f01acc_code = (String)detail_column.get(j);
            	 		   selectacc_code +="'"+f01acc_code+"'";            	
            	 		   //95.09.26 add=================================================================================================================             	 		   
            	 		   ori_field += f01acc_code;            	 		                            	 		                  	 		      
            	 		   //sum(decode(bal_lm_col,'A_1_bal_lm',bal_lm_value,0)) as a_1_bal_lm,
            	 		   if(f01_field_sum.indexOf(" ,sum(decode(acc_code,'"+f01acc_code.substring(4,f01acc_code.length())+"',"+f01acc_code.substring(0,f01acc_code.length()-7)+",0)) as "+f01acc_code) == -1){
            	 		      f01_field_sum += " ,sum(decode("+f01acc_code.substring(4,f01acc_code.length())+"_col,'"+f01acc_code.substring(0,1).toUpperCase()+f01acc_code.substring(1,f01acc_code.length())+"',"+f01acc_code.substring(4,f01acc_code.length())+"_value,0)) as "+f01acc_code; 							                
            	 		   }
            	 		   //sum(d.a_1_acct_cnt_tm) as a_1_acct_cnt_tm,
            	 		   if(f01_field_sum_hsien_id.indexOf(" ,sum(d."+f01acc_code+") as "+f01acc_code) == -1){
            	 		       f01_field_sum_hsien_id += " ,sum(d."+f01acc_code+") as "+f01acc_code;
            	 		   }            	 		   
            	 		   //f01.a_1_bal_lm
            	 		    //sum(d.a_1_acct_cnt_tm) as a_1_acct_cnt_tm,
            	 		   if(f01_field_hsien_id.indexOf(" ,f01."+f01acc_code) == -1){
            	 		      f01_field_hsien_id += " ,f01."+f01acc_code;
            	 		   }
            	 		   //====================================================================================================================================
			...
			prop_column_name.load(new FileInputStream(Utility.getProperties("schemaDir")+System.getProperty("file.separator")+"F01_6_column.TXT"));			
			//====================================================================================================
            ...
            select f01.bank_code,bn01.bank_name,m_year || '/' || m_month as m_yearmonth,
            	   wlx01.hsien_id,cd01.hsien_name,f01.a_1_acct_cnt_tm
            from (select m_year,m_month,bank_code,
                  		 sum(decode(acct_cnt_tm_col,'A_1_acct_cnt_tm',acct_cnt_tm_value,0)) as a_1_acct_cnt_tm,
            	   		 sum(decode(bal_lm_col,'A_1_bal_lm',bal_lm_value,0)) as a_1_bal_lm,
            	   		 sum(decode(acct_cnt_tm_col,'B_1_acct_cnt_tm',acct_cnt_tm_value,0)) as b_1_acct_cnt_tm,
            	   		 sum(decode(bal_lm_col,'B_1_bal_lm',bal_lm_value,0)) as b_1_bal_lm								 
            	 from (	
            	 		select M_YEAR, M_MONTH, BANK_CODE, bank_name,
            	   			   dep_type || '_' || acct_type || '_' || 'acct_cnt_tm' as acct_cnt_tm_col,
            	   			   sum(ACCT_CNT_TM) as acct_cnt_tm_value,
            	   			   dep_type || '_' || acct_type || '_' || 'bal_lm' as bal_lm_col,
            	   			   sum(bal_lm) as bal_lm_value,
            	   			   dep_type || '_' || acct_type || '_' || 'dep_tm' as dep_tm_col,
               	   			   sum(dep_tm) as dep_tm_value,
            	   			   dep_type || '_' || acct_type || '_' || 'wtd_tm' as wtd_tm_col,
               	   			   sum(wtd_tm) as wtd_tm_value,
            	   			   dep_type || '_' || acct_type || '_' || 'bal_tm' as bal_tm_col,
               	   			   sum(bal_tm) as bal_tm_value            	   			   
            			from (
              				   select M_YEAR, M_MONTH, BANK_CODE, bank_name, DEP_TYPE,  ACCT_TYPE,  
            	   					        sum(ACCT_CNT_TM)    AS  ACCT_CNT_TM,
            			  			  round(sum(BAL_LM)/1,0)    AS  BAL_LM, 
            			  			  round(SUM(DEP_TM)/ 1 ,0)  AS  DEP_TM, 
            			  			  round(SUM(WTD_TM)/ 1 ,0)  AS  WTD_TM, 
            			  			  round(SUM(BAL_TM)/ 1 ,0)  AS  BAL_TM  
            			  	   from   F01 ,  ba01  
            			  	   where (to_char(m_year * 100 + m_month) >= 9401 and to_char(m_year * 100 + m_month) <= 9402)
            			       and    F01.BANK_CODE  in ('6030016','6171342')  
            			       and    (f01.BANK_CODE =ba01.bank_no  and ba01.bank_type='6')  
            			       group  by  M_YEAR, M_MONTH, BANK_CODE, bank_name,  DEP_TYPE,  ACCT_TYPE                 
            			       ORDER  by  M_YEAR, M_MONTH, BANK_CODE, bank_name,  DEP_TYPE,  ACCT_TYPE 
            				 )
            			group by m_year,m_month,bank_code,bank_name
            				    ,dep_type || '_' || acct_type || '_' || 'acct_cnt_tm'
            					,dep_type || '_' || acct_type || '_' || 'bal_lm'					
            	    			,dep_type || '_' || acct_type || '_' || 'dep_tm'
            	    			,dep_type || '_' || acct_type || '_' || 'wtd_tm' 		   	   
            	    			,dep_type || '_' || acct_type || '_' || 'bal_tm'
            		  )		  
            		  group by m_year,m_month,bank_code
            	)f01,bn01 left join wlx01 on bn01.bank_no = wlx01.bank_no
            			  left join cd01 on wlx01.HSIEN_ID = cd01.HSIEN_ID  
            where f01.bank_code = bn01.bank_no
            and bn01.bank_no in ('6030016','6171342') and bn01.bn_type <> '2' 
            and f01.bank_code = bn01.bank_no		
            group by f01.bank_code,bn01.bank_name,f01.m_year,f01.m_month,wlx01.hsien_id,cd01.hsien_name,f01.a_1_acct_cnt_tm
            order by f01.bank_code,f01.m_year,f01.m_month
            
            //縣市別.按縣市.年月sort
            select e.FR001W_OUTPUT_ORDER,
            	   nvl(e.hsien_id,' ') as hsien_id,
            	   nvl(e.hsien_name,'其他') as hsien_name,
            	   m_year || decode(m_year,null,'','/') || m_month as m_yearmonth,		
             	   sum(d.a_1_acct_cnt_tm) as a_1_acct_cnt_tm,
            	   sum(d.a_1_bal_lm) as a_1_bal_lm
            from			 
            	 (select wlx01.hsien_id,cd01.hsien_name,f01.bank_code,bn01.bank_name,
            	         f01.m_year,f01.m_month,f01.a_1_acct_cnt_tm,f01.a_1_bal_lm
            	  from (select m_year,m_month,bank_code,
             				   sum(decode(acct_cnt_tm_col,'A_1_acct_cnt_tm',acct_cnt_tm_value,0)) as a_1_acct_cnt_tm,
             				   sum(decode(bal_lm_col,'A_1_bal_lm',bal_lm_value,0)) as a_1_bal_lm
            			from (	
            				  ...
            				  from (			
            	   					  select M_YEAR, M_MONTH, BANK_CODE, bank_name, DEP_TYPE,  ACCT_TYPE,  
            			  					 sum(ACCT_CNT_TM)  AS  ACCT_CNT_TM,
              			  					 ...
              			  					 round(SUM(BAL_TM)/ 1 ,0)       AS  BAL_TM  
              	   					 from   F01 ,  ba01  
              	   					 where (to_char(m_year * 100 + m_month) >= 9401 and to_char(m_year * 100 + m_month) <= 9412)
                   					 and    (f01.BANK_CODE =ba01.bank_no  and ba01.bank_type='6')  
                   					 group  by  M_YEAR, M_MONTH, BANK_CODE, bank_name,  DEP_TYPE,  ACCT_TYPE                 
                   					 ORDER  by  M_YEAR, M_MONTH, BANK_CODE, bank_name,  DEP_TYPE,  ACCT_TYPE 
            	 				  )
            ...
            //add 營運中/已裁撤============================             
			condition += " and bn01.bn_type "+(CANCEL_NO.equals("N")?"<>":"=")+" '2' and f01.bank_code = bn01.bank_no ";		  	 
			//======================================================
            ...
            StringBuffer f01_table= new StringBuffer();//f01
            List f01_tableList = new ArrayList() ;
            
            f01_table.append(" (	"	
			 		  + "  select m_year,m_month,bank_code"+ f01_field_sum		  				 		
				      + "  from ( "            	 	
				      + "		  select m_year, m_month,bank_code,bank_name,"
				      + "			     dep_type || '_' || acct_type || '_' || 'acct_cnt_tm' as acct_cnt_tm_col,"
	   			   	  + "		   		 sum(acct_cnt_tm) as acct_cnt_tm_value,"
				   	  ...
				   	  + "			   	 dep_type || '_' || acct_type || '_' || 'bal_tm' as bal_tm_col,"
  			   	   	  + "			   	 sum(bal_tm) as bal_tm_value"            	   			   
            		  + "		  from ( "
              		  + "			    select f01.m_year, f01.m_month, bank_code, bank_name, dep_type, acct_type," 
            	   	  + "					   sum(acct_cnt_tm)         as acct_cnt_tm,"
           			  + " 			  		   round(sum(bal_lm)/1,0)   as bal_lm,"
                      + "		  			   round(SUM(dep_tm)/ 1 ,0) as dep_tm,"
            		  + "	  			  	   round(SUM(wtd_tm)/ 1 ,0) as wtd_tm," 
            		  + "	  			 	   round(SUM(bal_tm)/ 1 ,0) as bal_tm"  
            		  + "	 	  	   from   f01 ,  (select * from ba01 where m_year = ? )ba01"  
            		  + "              where (to_char(f01.m_year * 100 + f01.m_month) >=? and to_char(f01.m_year * 100 + f01.m_month) <= ? )");
		    f01_tableList.add(u_year) ;
		    f01_tableList.add(S_YEAR+S_MONTH) ;
		    f01_tableList.add(E_YEAR+E_MONTH) ;
			if(hasBankListALL.equals("false")){	                                
			   f01_table.append(" 	 	   and   f01.bank_code in ("+selectBank_no+")");		                   
			 
			}
			if(bank_type.equals("ALL")){//95.10.27 add 農漁會
			   f01_table.append("      	   and  (f01.BANK_CODE =ba01.bank_no  and ba01.bank_type in ( ? , ? ))");  
			   f01_tableList.add("6") ;
			   f01_tableList.add("7") ;
			}else{//農 or 漁會               	
               f01_table.append("      	   and    (f01.BANK_CODE = ba01.bank_no  and ba01.bank_type in ('"+bank_type+"'))");  
            }
			f01_table.append("	     	   group  by  f01.m_year, f01.m_month, bank_code, bank_name, dep_type, acct_type"                 
            		  + "      	   ORDER  by  m_year, m_month, bank_code, bank_name, dep_type, acct_type" 
            		  + "		  	   ) "
            		  + "		 group by m_year,m_month,bank_code,bank_name " 
            		  + "				  ,dep_type || '_' || acct_type || '_' || 'acct_cnt_tm'"
           ...
           sqlCmd.append(" select f01.bank_code,bn01.bank_name,f01.m_year || '/' || f01.m_month as m_yearmonth," 
                   + "       wlx01.hsien_id,cd01.hsien_name,"+ori_field           			  
		           + " from "+f01_table.toString()+",").append(bn01.toString()).append(" bn01 left join ").append(wlx01.toString()).append(" wlx01 on bn01.bank_no = wlx01.bank_no "
			       + "                         left join ").append(cd01Table).append(" cd01 on wlx01.HSIEN_ID = cd01.HSIEN_ID " 
			       + " where f01.bank_code = bn01.bank_no "
			       + " and bn01.bank_no in ("+selectBank_no+")"
			       + " and bn01.bank_type= ? "
			       + " and f01.m_year is not null "
				   + " and f01.m_month is not null"
			       + condition
			       + " group by f01.bank_code,bn01.bank_name,f01.m_year,f01.m_month,wlx01.hsien_id,cd01.hsien_name,"+ori_field ); 
		   for(Object obj : f01_tableList) {
			   sqlCmdList.add(obj) ;
		   }
		   ...
		   sqlCmdList.add(bank_type) ;
			...
			sqlCmd_sum.append(" select e.FR001W_OUTPUT_ORDER,"
					   + "        nvl(e.hsien_id,' ') as hsien_id,"
	   			   	   ...
	   			   	   +f01_field_sum_hsien_id	   			  		 
					   + " from  (select wlx01.hsien_id,cd01.hsien_name,f01.bank_code,bn01.bank_name,"
					   + "				 f01.m_year,f01.m_month" + f01_field_hsien_id
  	  	  		  	   + " 		  from "+f01_table.toString()+",").append(bn01.toString()).append(" bn01 left join ").append(wlx01.toString()).append(" wlx01 on bn01.bank_no = wlx01.bank_no "
			           + "                		          left join ").append(cd01Table).append(" cd01 on wlx01.HSIEN_ID = cd01.HSIEN_ID " 
			           + " 		  where f01.bank_code = bn01.bank_no " + condition);
			for(Object obj : f01_tableList) {
			...
			sqlCmd_sumList.add(u_year) ;
			if(bank_type.equals("ALL")){//95.10.27 add 農漁會           
			   sqlCmd_sum.append(" and bn01.bank_type in (?,?)");
			   sqlCmd_sumList.add("6") ;
			   sqlCmd_sumList.add("7") ;
			}else{//農 or 漁會
			   sqlCmd_sum.append(" and bn01.bank_type in ('"+bank_type+"')");			  							
			}
			sqlCmd_sum.append("		 ) d , (select * from v_bank_location where m_year= ?)e ");
			sqlCmd_sumList.add(u_year); 
			if(bank_type.equals("ALL")){//95.10.27 add 農漁會           
			   sqlCmd_sum.append(" where e.bank_type in ( ? , ? )");
			   sqlCmd_sumList.add("6") ;
			   sqlCmd_sumList.add("7") ;
			}else{//農 or 漁會
			   sqlCmd_sum.append(" where e.bank_type in ('"+bank_type+"')");
			}
			sqlCmd_sum.append(" and d.bank_code(+)=e.bank_no "
					   + " and (e.hsien_id>'Y' or e.hsien_id<'Y')" 
			...
			if(!order.equals("")){	   
			    //100.05.12 fix 有挑選排序欄位年月時,查詢SQL error by 2295 						    
			    sqlCmd.append(" order by "+order.replaceAll("order_YYMM", "f01.m_year,m_month")); //各別機構 
			    sqlCmd_sum.append(" order by " + order.replaceAll("order_YYMM", "d.m_year,m_month"));//全部				    			    
  		       ...
  		       sqlCmd.append(" order by f01.bank_code,f01.m_year,f01.m_month ");
  		       sqlCmd_sum.append(" order by e.fr001w_output_order,d.m_year,m_month");
  		    }	       			      
            //System.out.println("sqlCmd="+sqlCmd);
            //讀取報表欄位名稱===================================================================================        	
        	Properties p = new Properties();
			p.load(new FileInputStream(Utility.getProperties("schemaDir")+System.getProperty("file.separator")+"F01_6_column.TXT"));			
			//====================================================================================================
            ...
            wb.setRepeatingRowsAndColumns(0, 1, columnLength+3, 1, 7); //設定表頭 為固定 先設欄的起始再設列的起始
			  ...
			  //System.out.println("DS007W_Excel.sqlCmd_sum="+sqlCmd_sum);	      
            ...
            for(i=0;i<dbData.size();i++){            
                ...
                for(int cellIdx =4;cellIdx < (new Short(lastCellNum)).intValue();cellIdx++){                    
                     ...
                     if(acc_code.indexOf("acct_cnt_tm") == -1){////該值不為戶數時.再除以單位4捨五入                     
