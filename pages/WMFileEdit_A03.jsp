...
//94.05.16 fix 只有會計科目為xxxP才檢查不可輸入小數點 by 2295
...
<%
	...
	System.out.println("WMFileEdit_A03.jsp");	
	...
	if(act.equals("new")){
   	    if(bank_type.equals("6")){//農會
   	       data_div01 = Utility.getAcc_Code("ncacno","A03","05");//平均利率     	    
       }else{//漁會
           data_div01 = Utility.getAcc_Code("ncacno_7","A03","05");//平均利率            
	   }
	}else{
	...
	if(permission == null){
       System.out.println("WMFileEdit_A03.permission == null");
    }else{
       System.out.println("WMFileEdit_A03.permission.size ="+permission.size());
                    ...
                    <tr> 
                      ...
                      <td width="300"><font color='#000000' size=4><b> 
                        <center>
                          ...
                          <%}else{%>
                            <font color='#000000' size=4>線上編輯</font><font color="#CC0000">【<font size=4><%=ListArray.getDLIdName("1", "A03")%>】</font></font><font color='#000000' size=4></font> 
                          <%}%>  
                          </center>
                          </b> 
                        </center>
                    ...
                    <tr> 
                      <td><Table width=600 border=1 align=center cellpadding="1" cellspacing="1" bordercolor="#3A9D99">
                          <%if(act.equals("Query")){%>
                            ...
                            <td colspan=2 bgcolor='e7e7e7'>A03&nbsp;&nbsp;&nbsp;<%=ListArray.getDLIdName("1", "A03")%></td>
                          </tr>  
                          <%}%>
...
%>
		    ...
		    <%if(act.equals("new")){%>
		    	<input type='text' name='amt' value="" size=16 maxlength=16 onFocus='this.value=changeVal(this)' onBlur='<%if(!tmpAcc_Code.substring(tmpAcc_Code.length()-1).equals("P")){out.print("checkPoint_focus(this);");}%>this.value=changeStr(this)' style='text-align: right;'>
		    <%}else{
		    	if( ((DataObject)data_div01.get(i)).getValue("amt") == null ||  (((DataObject)data_div01.get(i)).getValue("amt") != null && ((((DataObject)data_div01.get(i)).getValue("amt")).toString()).equals("0")) ){%>
		    		<input type='text' name='amt' value="" size=16 maxlength=16 onFocus='this.value=changeVal(this)' onBlur='<%if(!tmpAcc_Code.substring(tmpAcc_Code.length()-1).equals("P")){out.print("checkPoint_focus(this);");}%>this.value=changeStr(this)' style='text-align: right;'>
		    	<%}else{%>
					<input type='text' name='amt' value="<%=tmpAcc_Code.substring(tmpAcc_Code.length()-1).equals("P")?Utility.getPercentNumber(((((DataObject)data_div01.get(i)).getValue("amt")) == null ? "":(((DataObject)data_div01.get(i)).getValue("amt"))).toString()) : Utility.setCommaFormat(((((DataObject)data_div01.get(i)).getValue("amt")) == null ? "":(((DataObject)data_div01.get(i)).getValue("amt"))).toString())%>" size=16 maxlength=16 onFocus='this.value=changeVal(this)' onBlur='<%if(!tmpAcc_Code.substring(tmpAcc_Code.length()-1).equals("P")){out.print("checkPoint_focus(this);");}%>this.value=changeStr(this)' style='text-align: right;'>
				<%}%>
			<%}%>
			</a>
              ...
              <tr> 
				...
				<%if(act.equals("new")){%>     
				     <%if(permission != null && permission.get("A") != null && permission.get("A").equals("Y")){ //add%>                   	        	                                   		       
                        <td width="66"> <div align="center"><a href="javascript:doSubmit(this.document.forms[0],'Insert','A03','','','<%=bank_type%>');" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('Image101','','images/bt_confirmb.gif',1)"><img src="images/bt_confirm.gif" name="Image101" width="66" height="25" border="0" id="Image101"></a></div></td>                        
                     <%}%>   
         		<%}%>
         		<%if(act.equals("Edit")){%>
         		     <%if(permission != null && permission.get("U") != null && permission.get("U").equals("Y")){ //update%>                   	        	                                   		     
				        <td width="66"> <div align="center"><a href="javascript:doSubmit(this.document.forms[0],'Update','A03','<%=S_YEAR%>','<%=S_MONTH%>','<%=bank_type%>');" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('Image102','','images/bt_updateb.gif',1)"><img src="images/bt_update.gif" name="Image102" width="66" height="25" border="0" id="Image102"></a></div></td>			            
				     <%}%>   
				     <%if(permission != null && permission.get("D") != null && permission.get("D").equals("Y")){ //delete%>                   	        	                                   		     
				        <td width="66"> <div align="center"><a href="javascript:doSubmit(this.document.forms[0],'Delete','A03','<%=S_YEAR%>','<%=S_MONTH%>','<%=bank_type%>');" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('Image103','','images/bt_deleteb.gif',1)"><img src="images/bt_delete.gif" name="Image103" width="66" height="25" border="0" id="Image103"></a></div></td>										               
				     <%}%>   
				<%}%>				
         		<%if(!act.equals("Query")){%>       
              ...
              <tr> 
                <td><table width="600" border="0" cellpadding="1" cellspacing="1" class="sbody">
                    ...
                    <tr> 
                      <td width="16">&nbsp;</td>
                      <td width="577"> <ul>
                          <li>本網頁提供新增<%=ListArray.getDLIdName("1", "A03")%>。</li>
