...
//105.11.18 add 增加顯示協助措施名稱 by 2295
%>
                ...
                <tr><div align="right"><jsp:include page="getLoginUser.jsp?width=770" flush="true" /></div></tr>                  
                    <tr class="sbody">
                      ...
                      <table class="sbody" width="770" border=1 cellpadding="0" cellspacing="1" bordercolor="#3A9D99" height="20" >
                          <%if(dbData.size()==0){%>
                          <tr>
                            <td width="762" align="center" colspan="2" bgcolor="#FFFFE6">無適用的協助措施</td>
                          </tr>
                          <%}else{%>
                          	 <table class="sbody" width="770" border=1 cellpadding="0" cellspacing="1" bordercolor="#3A9D99" height="20" >      
                          	 <tr>
                              <td width="150" bgcolor="#D2F0FF" align="center" >協助措施名稱</td>
                              <td width="*" bgcolor="#D2F0FF" align="center" ><%=((DataObject)dbData.get(0)).getValue("acc_tr_name")%></td>
                             </tr>
                             </table>
                        <table class="sbody" width="770" border=1 cellpadding="0" cellspacing="1" bordercolor="#3A9D99" height="20" >      
                           <tr>
                            <td width="300" bgcolor="#B1DEDC" align="center" >申報基準日</td>
                            <td width="462" bgcolor="#B1DEDC" align="center" >申報日期</td>
                          </tr> 
                          <%	  
                              for(int i=0;i<dbData.size();i++){
                            	  String goPage= "new";
                            	  String applyDate = String.valueOf(((DataObject)dbData.get(i)).getValue("applydate")).substring(0, 10);
                            	  String tmpDate = String.valueOf(((DataObject)dbData.get(i)).getValue("add_date"))=="null"?"&nbsp;":
                          		  					String.valueOf(((DataObject)dbData.get(i)).getValue("add_date"));
                            	  String add_Date = tmpDate;
                            	  String add_DateHHMMSS ="&nbsp";
                            	  if(!"&nbsp".equals(tmpDate) && tmpDate.length()>=10){
                            		  add_Date = tmpDate.substring(0, 10);
                            		  add_DateHHMMSS = tmpDate.substring(10,tmpDate.length()-2);
                            		  goPage= "Edit";
                            	  }
                          ...
                          %>
                           ...
                           <tr class="sbody" bgcolor="<%out.print((i%2==0)?"#FFFFE6":"#F2F2F2");%>">
                          	   <td class="sbody" width="300" align="center">                            
	                           <a href='TM006W.jsp?act=<%=goPage %>&acc_Tr_Type=<%=acc_Tr_Type %>&applyDate=<%=applyDate%>'>
	                           <%=Utility.getCHTdate(applyDate,0)%></a></td>  
	                           <td class="sbody" width="462" align="center">                            
	                           <%=Utility.getCHTdate(add_Date,0)%><%=add_DateHHMMSS %></td>           
                           </tr>                 
                          <%  }//end for
                            }//end else %>
                         </table>                     
