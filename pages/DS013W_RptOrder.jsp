...
//95.11.13 add 區分BOAF.MIS配色 by 2295
//95.11.16 add 拿掉年sort by 2295
//95.12.06 add 各別機構可根據縣市別.金融機構代號.年月sort
//             全部可選根據縣市別小計.年月sort by 2295
//         add 起始日期不可大於結束日期
//95.12.07 add 若類別為地方主管機關.可根據地方主管機關代號sort
//99.05.25 fix 排序欄位 改共用 by 2808
%>
...
<%
	...
	System.out.println("DS013W_RptOrder.szExcelAction="+szExcelAction);
        ...
        <tr> 
          <td><table width="750" border="0" align="center" cellpadding="0" cellspacing="0">
              <tr> 
                ...
                <td width="*" class="title_font">承受擔保品延長處分申報情形資料 </td>
                <td width="220"><img src="images/banner_bg1.gif" width="220" height="17"></td>
              </tr>
            </table></td>
        </tr>
                    ...
                    <tr> 
                     <td class="body_bgcolor">
                      <table width="750" border="0" cellpadding="1" cellspacing="1">                       
                       <tr>                           
                        <td width="750" align=left><font color="red" size=2>註：「承受擔保品延長處分申報情形明細資料」本排序欄位若未選取，則按「縣市別」、「地方主管機關代號名稱」、</font></td>                              
                       </tr>                              
                       <tr>                           
                        <td width="750" align=left><font color="red" size=2>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;「金融機構代號名稱」及「年季」等欄位排序印出。</font></td>                              
                       </tr> 
                      </table>
                     </td>
                    </tr>
...
<INPUT type="hidden" name=rptBank_Type value='<%if(session.getAttribute("nowbank_type") != null) out.print((String)session.getAttribute("nowbank_type"));%>'><!--//95.12.07儲存已勾選的機構類別-->
</form>
...
if(session.getAttribute("btnFieldList") != null && !((String)session.getAttribute("btnFieldList")).equals("")){ 
   System.out.println("DS013W_RptOrder.btnFieldList="+(String)session.getAttribute("btnFieldList"));
%>
<%}%>
getbtnFieldList();
function getbtnFieldList(){
	...
	if(this.document.forms[0].BankList.value.indexOf("ALL") == -1){
	   ...
	   if(this.document.forms[0].rptBank_Type.value =='B'){//95.12.07 add 若類別為地方主管機關.可根據地方主管機關代號sort
	     //地方主管機關代號名稱============================================================================================
	     checkAdd = false;
	     for(var k =0;k<this.document.forms[0].SortListDst.length;k++){			
	       if(this.document.forms[0].SortListDst.options[k].text == "地方主管機關代號名稱"){		    
		    checkAdd = true;			       
	       }  
	     }    
         if(checkAdd == false){	  
            this.document.forms[0].SortListSrc.options[addCount] = new Option("地方主管機關代號名稱", "m2_name");
            addCount++;
         }
	   }
       ...
       if(checkAdd == false){	  
          this.document.forms[0].SortListSrc.options[addCount] = new Option("金融機構代號名稱", "wlx08_s_gage.bank_no");
          addCount++;
       }
       //年.月===================================================================================================
       checkAdd = false;
       for(var k =0;k<this.document.forms[0].SortListDst.length;k++){			
	       if(this.document.forms[0].SortListDst.options[k].text == "年季"){		       
		      checkAdd = true;			       
	       }  
	   } 	    
	   if(checkAdd == false){	  
	      this.document.forms[0].SortListSrc.options[addCount] = new Option("年季", " m_yearmonth");
	      addCount++;
	   }	
    }else{//已選的機構代號=ALL全部
       //縣市別小計============================================================================================
