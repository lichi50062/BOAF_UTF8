...
//104.02.12 fix A12 js by 2295
//104.05.22 fix 增加檢核若不為0且<1000時,顯示錯誤訊息 by 2295
%>
...
<%
	...
	System.out.println("WMFileEdit_A12.jsp");	
	...
	System.out.println("WMFileEdit_A12.act="+act);
	...
	if(data_div01 != null && data_div01.size() != 0){	 		
		...
		baddebt_Amt=(bean.getValue("baddebt_amt") == null)?"0":(bean.getValue("baddebt_amt")).toString();
		loss_Amt=(bean.getValue("loss_amt") == null)?"0":(bean.getValue("loss_amt")).toString();
		profit_Amt=(bean.getValue("profit_amt") == null)?"0":(bean.getValue("profit_amt")).toString();
	...
	if(permission == null){
       System.out.println("WMFileEdit_A12.permission == null");
    }else{
       System.out.println("WMFileEdit_A12.permission.size ="+permission.size());
                    ...
                    <tr> 
                      ...
                      <td width="*"><font color='#000000' size=4><b> 
                        <center>
                          ...
                          <%}else{%>
                            <font color='#000000' size=4>線上編輯</font><font color="#CC0000">【<font size=4><%=ListArray.getDLIdName("1", "A12")%>】</font></font><font color='#000000' size=4></font> 
                          <%}%>  
                          </center>
                          </b> 
                        </center>
                    ...
                    <tr> 
                      <td><Table width=600 border=1 align=center cellpadding="1" cellspacing="1" bordercolor="#3A9D99">
                          <%if(act.equals("Query")){%>
                            ...
                            <td colspan=2 bgcolor='e7e7e7'>A12&nbsp;&nbsp;&nbsp;<%=ListArray.getDLIdName("1", "A12")%></td>
                          </tr>  
                          <%}%>
                          <tr class="sbody"> 
                            <td bgcolor="#D2F0FF" width="260"> <div align=left>申報年月</div></td>
                            <td bgcolor='e7e7e7'>
                            <input type='text' name='S_YEAR' value="<%=S_YEAR%>" <%if(act.equals("Edit")) out.print("disabled");%> size='3' maxlength='3' onblur='CheckYear(this)'>
        						...
        						<%if(act.equals("Edit")){%>
        						 	<a href="javascript:doSubmit(this.document.forms[0],'Print','A12','<%=S_YEAR%>','<%=S_MONTH%>','');" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('Image105','','images/bt_print.gif',1)"><img src="images/bt_print.gif" name="Image105" width="66" height="25" border="0" id="Image105"></a>                     
        						<%} %>
                            </td>
                          </tr>
						 ...
						 <tr class="sbody">	
						    <td bgcolor="#D2F0FF" width="260">							
							   <div align=left>轉銷呆帳金額-本月減少備抵呆帳</div>
			                </td>
			                <td bgcolor="#e7e7e7">							
			                   <input type='text' name='baddebt_Amt' value="<%=Utility.setCommaFormat(baddebt_Amt)%>" size=16 maxlength=16 onFocus='this.value=changeVal(this)' onBlur='checkPoint_focus(this);this.value=changeStr(this);' style='text-align: right;<%if(Integer.parseInt(baddebt_Amt.equals("")?"0":baddebt_Amt) < 0) out.print(" color:red;");%>'>元
			                </td>						
			            </tr>
			            <tr class="sbody">	
						    <td bgcolor="#D2F0FF" width="260">							
							   <div align=left>轉銷呆帳金額-本月直接認列損失</div>
			                </td>
			                <td bgcolor="#e7e7e7">							
			                   <input type='text' name='loss_Amt' value="<%=Utility.setCommaFormat(loss_Amt)%>" size=16 maxlength=16 onFocus='this.value=changeVal(this)' onBlur='checkPoint_focus(this);this.value=changeStr(this);' style='text-align: right;<%if(Integer.parseInt(loss_Amt.equals("")?"0":loss_Amt) < 0) out.print(" color:red;");%>'>元
			                </td>						
			            </tr>
			           
			            <tr class="sbody">	
						    <td bgcolor="#D2F0FF" width="260">							
							   <div align=left>存款準備率降低所增加盈餘(本月增加金額)</div>
			                </td>
			                <td bgcolor="#e7e7e7">							
			                   <input type='text' name='profit_Amt' value="<%=Utility.setCommaFormat(profit_Amt)%>" size=16 maxlength=16 onFocus='this.value=changeVal(this)' onBlur='checkPoint_focus(this);this.value=changeStr(this);' style='text-align: right;<%if(Integer.parseInt(profit_Amt.equals("")?"0":profit_Amt) < 0) out.print(" color:red;");%>'>元
			                	<font color="#FF0000">請注意本表金額單位為元</font>
			                </td>						
			            </tr>
	          </Table></td>
                    </tr>
              ...
              <tr> 
				...
				<%if(act.equals("new")){%>     
				     <%if(permission != null && permission.get("A") != null && permission.get("A").equals("Y")){ //add%>                   	        	                                   		       
                        <td width="66"> <div align="center"><a href="javascript:doSubmit(this.document.forms[0],'Insert','A12','','','<%=bank_type%>');" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('Image101','','images/bt_confirmb.gif',1)"><img src="images/bt_confirm.gif" name="Image101" width="66" height="25" border="0" id="Image101"></a></div></td>                        
                     <%}%>   
         		<%}%>
         		<%if(act.equals("Edit")){%>
         		     <%if(permission != null && permission.get("U") != null && permission.get("U").equals("Y")){ //update%>                   	        	                                   		     
				        <td width="66"> <div align="center"><a href="javascript:doSubmit(this.document.forms[0],'Update','A12','<%=S_YEAR%>','<%=S_MONTH%>','<%=bank_type%>');" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('Image102','','images/bt_updateb.gif',1)"><img src="images/bt_update.gif" name="Image102" width="66" height="25" border="0" id="Image102"></a></div></td>			            
				     <%}%>   
				     <%if(permission != null && permission.get("D") != null && permission.get("D").equals("Y")){ //delete%>                   	        	                                   		     
				        <td width="66"> <div align="center"><a href="javascript:doSubmit(this.document.forms[0],'Delete','A12','<%=S_YEAR%>','<%=S_MONTH%>','<%=bank_type%>');" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('Image103','','images/bt_deleteb.gif',1)"><img src="images/bt_delete.gif" name="Image103" width="66" height="25" border="0" id="Image103"></a></div></td>										               
				     <%}%>   
				<%}%>				
         		<%if(!act.equals("Query")){%>       
              ...
              <tr> 
                <td><table width="600" border="0" cellpadding="1" cellspacing="1" class="sbody">
                    ...
                    <tr> 
                      <td width="16">&nbsp;</td>
                      <td width="648"> <ul>
                          <li>本網頁提供新增每月轉銷呆帳及存款準備率降低所增盈餘資料。</li>
							<li><font color="#FF0000">
							存款準備率降低所增加之盈餘，「本月增加金額」請以88年1月份應提準備存款乘以調降幅度後，乘以（存放中央銀行或實際運用之利率 
							-【準備金乙戶加權利率 × 準備金乙戶成數】）÷ 12設算。</font></li>
							<li><font color="#FF0000">
							自97年4月底之資料，上述準備金乙戶加權利率（R）依下列公式計算：R = R1*D1/D + 
							R2*D2/D，其中R為準備金乙戶加權利率；R1為準備金乙戶之定期性存款利率、R2為準備金乙戶之活期性存款利率；D為前一期法定準備額，其中D1為前一期法定準備額之定期性存款部分、D2為前一期法定準備額之活期性存款部分。</font></li>
