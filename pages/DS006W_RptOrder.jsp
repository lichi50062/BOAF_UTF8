...
// 95.10.23 add order by 逾放合計 by 2295
// 95.11.14 add 區分BOAF.MIS配色 by 2295
// 95.11.16 add 拿掉年sort by 2295
// 95.12.05 add 各別機構可根據縣市別.金融機構代號.年月sort
//              全部可選根據縣市別小計.年月sort by 2295
//          add 起始日期不可大於結束日期
// 99.05.17 fix 排序欄位 改共用 by 2808
//100.05.12 fix 有挑選排序欄位年月時,查詢SQL error by 2295
//103.02.11 add 漁會科目代號新舊無法同時列印 by 2295
%>
...
<%
	...
	System.out.println("DS006W_RptOrder.szExcelAction="+szExcelAction);
...
function doSubmit(cnd){
    if(cnd == 'createRpt'){
      ...
      if(!chInput(this.document.forms[0],'')) return;//95.12.05 add 起始日期不可大於結束日期
      //漁會科目代號新舊無法同時列印    
      if(this.document.forms[0].DS_bank_type.value == '7'){
        ...
        <tr> 
          <td><table width="750" border="0" align="center" cellpadding="0" cellspacing="0">
              <tr> 
                ...
                <td width="*" class="title_font">A06逾期放款資料 </td>
                <td width="240"><img src="images/banner_bg1.gif" width="240" height="17"></td>
              </tr>
            </table></td>
        </tr>
                    ...
                    <%@include file="./include/DS_RptOrder.include" %><!-- 排序欄位.可挑選項目-->
                     ...
                     <td class="body_bgcolor">
                      <table width="750" border="0" cellpadding="1" cellspacing="1">
                       ...
                       <tr>                           
                        <td width="750" align=left><font color="red" size=2>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;「逾期放款明細資料」本排序欄位若未選取，則按「縣市別」、「金融機構代號名稱」及「年月」等欄位排序印出。</font></td>                              
                       </tr>                              
                      </table>
                     </td>
                    </tr>
...
<INPUT type="hidden" name=S_YEAR value='<%if(session.getAttribute("S_YEAR") != null) out.print((String)session.getAttribute("S_YEAR"));%>'><!--//95.12.05儲存已勾選的起始日期-年-->
<INPUT type="hidden" name=E_YEAR value='<%if(session.getAttribute("E_YEAR") != null) out.print((String)session.getAttribute("E_YEAR"));%>'><!--//95.12.05儲存已勾選的結束日期-年-->
<INPUT type="hidden" name=S_MONTH value='<%if(session.getAttribute("S_MONTH") != null) out.print((String)session.getAttribute("S_MONTH"));%>'><!--//95.12.05儲存已勾選的起始日期-月-->
<INPUT type="hidden" name=E_MONTH value='<%if(session.getAttribute("E_MONTH") != null) out.print((String)session.getAttribute("E_MONTH"));%>'><!--//95.12.05儲存已勾選的結束日期-月-->
<INPUT type="hidden" name=DS_bank_type value='<%if(session.getAttribute("DS_bank_type") != null) out.print((String)session.getAttribute("DS_bank_type"));%>'><!--//103.02.11儲存已勾選的結束日期-月-->
</form>
...
if(session.getAttribute("btnFieldList") != null && !((String)session.getAttribute("btnFieldList")).equals("")){ 
   System.out.println("DS006W_RptOrder.btnFieldList="+(String)session.getAttribute("btnFieldList"));
%>
<%}%>
getbtnFieldList();
function getbtnFieldList(){
	...
	if(this.document.forms[0].BankList.value.indexOf("ALL") == -1){	   
       ...
       if(checkAdd == false){	  
          this.document.forms[0].SortListSrc.options[addCount] = new Option("金融機構代號名稱", "a06.bank_code");
          addCount++;
       }       
       //年.月===================================================================================================
       checkAdd = false;
    ...
    //======================================================================================================
    //已選的報表欄位.95.12.05 不可根據已選欄位做sort
    /*
	for (var i =0; i < a.length; i ++){
		var j = a[i].split('+');
		checkAdd=false;		
    	...
    	if(checkAdd == false){	       	       
    	    ...
    	    //   continue;
    	    //}
    	    //95.10.23 add order by 逾放合計.
       		this.document.forms[0].SortListSrc.options[addCount] = new Option(j[1], "amt_total_"+j[0]);
       		addCount++;
    	}	
	}*/	
}
