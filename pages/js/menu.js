...
function Menu()
{
	//this.bgColor     = "#3177BB";
	this.bgColor     = "#9AD3D0";
	if (ie) this.menuFont = "x-small Verdana";
...
//為了處理select elements無法隱藏於layer之下的問題
//因效能問題，先行處理必須被隱藏的欄位，註記起來的code可以將整個頁面的select elements全部隱藏
function hideElement(elmID) {
  //Loop through all elements to check for overlay
  /*
  for (i = 0; i < document.all.tags(elmID).length; i++)
  */
  var count = document.all.tags(elmID).length;
  if ( count > 6 ) count = 6;
  for (i = 0; i < count; i++)
  {
    ...
    if (myObj.id == 'hide1') {
      if(ie) { myObj.style.visibility = 'hidden' }
      if(ns) { myObj.visibility = 'hide' }
    }
  }
}
