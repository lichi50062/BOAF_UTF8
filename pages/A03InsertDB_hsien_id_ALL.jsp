<%
// 95.09.29 add 將農.漁會縣市別小計寫入a03_opeartion by 2295
...
//102.06.24 add 改寫原本查詢SQL.並合併農.漁會縣市小計 by 2295
//102.06.24 add 103/01以後.A01漁會.套用新科目代號/計算公式 by 2295
%>
...
try{
	  ...
	  if(!logDir.exists()){
	   ...
	   logps.println(logformat.format(nowlog)+" "+"產生"+s_year+"年"+s_month+"月 (農.漁會-縣市別小計)A03_opeation 資料中");
	   logps.flush();
      ...
      //按縣市別
	  //農.漁會
sqlCmd.append(" select distinct f.bank_type,g.hsien_id,g.hsien_name,fr001w_output_order, ");
sqlCmd.append("        fieldb,fieldc,fieldd,fielde,fieldf,fieldg,fieldh,fieldi,fieldj,   ");
sqlCmd.append("        fieldk,fieldl,fieldm,fieldn,fieldo,fieldp,fieldq,fieldr,fields,   ");
sqlCmd.append("        fieldt,fieldu,fieldv,fieldw,fieldx,fieldy,fieldz,fieldza,fieldzb  ");
sqlCmd.append(" from (select d.bank_type,nvl(e.hsien_id,' ') as hsien_id,nvl(e.hsien_name,'其他') as hsien_name,");
sqlCmd.append("          round(sum(field_b)/ 1,0) as fieldB ,");
sqlCmd.append("          round(sum(field_c)/ 1,0) as fieldC ,");
sqlCmd.append("          round(sum(field_d)/ 1,0) as fieldD ,");
sqlCmd.append("          round(sum(field_e)/ 1,0) as fieldE ,");
sqlCmd.append("          round(sum(field_f)/ 1,0) as fieldF ,");
sqlCmd.append("          round(sum(field_g)/ 1,0) as fieldG ,");
...
sqlCmd.append("          round(sum(field_r) / 1,0) as fieldR ,  round(sum(field_s) / 1,0) as fieldS ,");
sqlCmd.append("          round(sum(field_t) / 1,0) as fieldT ,  round(sum(field_u) / 1,0) as fieldU ,");
sqlCmd.append("          round(sum(field_v) / 1,0) as fieldV ,  round(sum(field_w) / 1,0) as fieldW ,");
sqlCmd.append("          round(sum(field_x) / 1,0) as fieldX ,  round(sum(field_y) / 1,0) as fieldY ,");
sqlCmd.append("          round(sum(field_z) / 1,0) as fieldZ ,  round(sum(field_za)/ 1,0) as fieldZA,");
sqlCmd.append("          round(sum(field_zb)/ 1,0) as fieldZB       ");
sqlCmd.append("       from ( select a01.bank_type,a01.bank_code,a01.bank_name, ");
...
sqlCmd.append("              from ( select bank_type,bank_code,bank_name, ");
...
sqlCmd.append("                       and acc_code in ('420120','420140','420170','520110','520130','840640','520120','520140','840670')");
sqlCmd.append("                       )a03 ");
sqlCmd.append("                       left join (select * from ba01 where m_year=?)ba01 on a03.bank_code = ba01.bank_no");
paramList.add(wlx01_m_year);
...
paramList.add(wlx01_m_year);
sqlCmd.append("       where e.bank_type in ('6','7') ");
sqlCmd.append("       and d.bank_code(+)=e.bank_no   ");
sqlCmd.append("       and (e.hsien_id>'Y' or e.hsien_id<'Y') ");
sqlCmd.append("       group by d.bank_type,nvl(e.hsien_id,' '),nvl(e.hsien_name,'其他'),e.fr001w_output_order ");
sqlCmd.append("    ) f,(select * from v_bank_location where m_year=?) g ");
paramList.add(wlx01_m_year);
sqlCmd.append(" where f.hsien_id(+)=g.hsien_id ");
sqlCmd.append(" and f.bank_type is not null ");
sqlCmd.append(" order by f.bank_type,g.fr001w_output_order ");
    ...
    for(int i=0;i<dbData.size();i++){
        ...
        logcalendar = Calendar.getInstance();
	    nowlog = logcalendar.getTime();
	    logps.println(logformat.format(nowlog)+" "+"農漁會別:"+(String)bean.getValue("bank_type")+";縣市別:"+(String)((DataObject)dbData.get(i)).getValue("hsien_id"));
	    logps.flush();
