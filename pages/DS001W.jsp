<%
// 95.08.15-25 create by 2295 
// 95.08.30 fix 條件拿掉報表類別增加農/漁會別,報表欄位增加單位數,縣市別小計,排序欄位增加年.報表欄位 by 2295
// 95.09.06 add 若金融機構代號選為全部時,則讀取bank_type=ALL的設定檔 by 2295
//          add 此次所選的bank_type與上次不同清除已勾選的勾選的報表欄位/sort報表欄位 by 2295
// 95.11.03 fix 使用reportUtil執行儲存報表格式檔.讀取報表格式檔.刪除報表格式檔.取得範本資料 by 2295
// 		   fix 使用Utility.CheckPermission檢核權限 by 2295
// 95.11.10 add 儲存DS_bank_type. menu的機構類別 by 2295
...
<%
    ...
    if(!Utility.CheckPermission(request,report_no)){//無權限時,導向到LoginError.jsp//95.11.03 fix 使用Utility.CheckPermission檢核權限 by 2295
        ...
        if(act.equals("RptColumn")){//報表欄位              
			//List FieldList = Utility.getFileData(Utility.getProperties("schemaDir")+System.getProperty("file.separator")+"A01_"+bank_type+"_master_"+acc_div+".TXT");						
			//System.out.println(Utility.getProperties("schemaDir")+System.getProperty("file.separator")+"A01_"+bank_type+"_master_"+acc_div+".TXT");
			//102.11.11 add 103/01以後,漁會套用新表格(增加/異動科目代號) 
			String strYymm="";
    		if( bank_type.equals("7") && (Integer.parseInt(Utility.getTrimString(dataMap.get("S_YEAR"))) * 100 + Integer.parseInt(Utility.getTrimString(dataMap.get("S_MONTH"))) >= 10301) ){
    			strYymm = "_10301";
    		}
			List FieldList = Utility.getFileData(Utility.getProperties("schemaDir")+System.getProperty("file.separator")+"A01_"+bank_type+"_master"+strYymm+".TXT");						
			//95.09.06 add 若金融機構代號選為全部時,則讀取bank_type=ALL的設定檔
			System.out.println(report_no+".bank_type="+bank_type);
			System.out.println((String)session.getAttribute("BankList"));
            if(!bank_type.equals("ALL") && session.getAttribute("BankList") != null && ((String)session.getAttribute("BankList")).indexOf("ALL") != -1){
              FieldList = Utility.getFileData(Utility.getProperties("schemaDir")+System.getProperty("file.separator")+"A01_ALL_master.TXT");						
            }
			System.out.println(Utility.getProperties("schemaDir")+System.getProperty("file.separator")+"A01_"+bank_type+"_master.TXT");
...
<%!
    private final static String report_no = "DS001W";
