<%
// 95.10.17 add 將農.漁會明細寫入a04_opeartion by 2295
...
public String Generate(String Report_no,String s_year,String s_month,String isDebug,String lguser_id) throws Exception{
 ...
 try{
	   ...
	   logps.println(logformat.format(nowlog)+" "+"產生"+s_year+"年"+s_month+"月 (農.漁會-各別機構)A04_opeation 資料中");
	   logps.flush();
      ...
      //各別機構
	  //農會.漁會
sqlCmd.append("  select a04.hsien_id,a04.hsien_name,a04.FR001W_output_order,a04.bank_type,a04.bank_no ,a04.BANK_NAME,                                       ");
sqlCmd.append("	    	(CASE WHEN (field_OVER_A01 = field_OVER_A04 ) THEN '   ' ELSE '***'  END)  as OVER_TYPE_1,                                          ");
sqlCmd.append("	    	(CASE WHEN (field_CREDIT_A01 = field_CREDIT_A04 ) THEN '   ' ELSE '***'  END)  as CREDIT_TYPE_1,                                    ");
...
sqlCmd.append("   		 group by nvl(cd01.hsien_id,' '),nvl(cd01.hsien_name,'OTHER'),cd01.FR001W_output_order,                                             ");
sqlCmd.append("   		 			bn01.bank_type,bn01.bank_no , bn01.BANK_NAME) a04                                                                       ");
...
paramList.add(s_month);
	...
	String[] field_type = {"2","2","4","0","0","4","0","4","4","0","0","0","0","0","0","0"};
    ...
    for(int i=0;i<dbData.size();i++){
		...
		for(int j=0;j<field.length;j++){
		    ...
		    if(field[j].equals("field_over_a01")){//1.field_OVER_A01=逾期放款[990000]
		       if(((String)bean.getValue("over_type_1")).equals("***")){
		          dataList.add("0");
		       }else{
		          dataList.add("2");
		       }
		    ...
		    }else if(field[j].equals("field_credit_a01")){//2.field_CREDIT_A01=總放款(放款含催收)[120000+120800+150300]
		       if(((String)bean.getValue("credit_type_1")).equals("***")){
		          dataList.add("Y");
		       }else{
		          dataList.add("N");
		       }
