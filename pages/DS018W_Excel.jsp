...
<%
	...
	try{
		...
		wlx01.append(" HSIEN_DIV_1,CANCEL_NO,CANCEL_DATE from wlx01 where m_year =? )");
			...
			prop_column.load(new FileInputStream(Utility.getProperties("schemaDir")+System.getProperty("file.separator")+"A09_ALL_detail.TXT"));
			//=======================================================================================================================
			        ...
			        if(detail_column != null && detail_column.size() != 0){
			           columnLength += detail_column.size();//累加總欄位個數
              		   for(j=0;j<detail_column.size();j++){
            	 		   ...
            	 		   selectacc_code +="'"+a09acc_code+"'";
            	 		   //95.09.26 add=================================================================================================================
            	 		   ori_field += a09acc_code;
            	 		   //====================================================================================================================================
			...
			prop_column_name.load(new FileInputStream(Utility.getProperties("schemaDir")+System.getProperty("file.separator")+"A09_ALL_column.TXT"));
			//====================================================================================================
            ...
            (
              select bn01.bank_no,bn01.bank_name,
                     ...
                     over_cnt,
                     Round(over_amt/1,0) as over_amt,
            	  	 Round(push_over_amt/1,0)  as push_over_amt,
            	  	 Round(totalamt/1,0)  as totalamt,
            	  	 Round(push_totalamt/1 ,0) as push_totalamt,
                	 over_total_rate
              ...
              group by bn01.bank_no,bn01.bank_name,
                       ...
                       over_cnt,Round(over_amt/1,0),Round(push_over_amt/1,0),Round(totalamt/1,0),
            	  	   Round(push_totalamt/1 ,0),over_total_rate
            ) A09
            ...
            select  cd01.FR001W_output_order as FR001W_output_order,
            		...
            		m_year || '/' || decode(substr(to_char(m_month),2),'','0','') || decode(substr(to_char(m_month),1),'1','1',substr(to_char(m_month),1)) as m_yearmonth,
            		m_year,m_month,
            		sum(A09.over_cnt)  as over_cnt,
            		Round(sum(A09.over_amt)/1,0)  as over_amt,
            		Round(sum(A09.push_over_amt)/1,0)  as push_over_amt,
            		Round(sum(A09.totalamt)/1,0)  as totalamt,
                    Round(sum(A09.push_totalamt)/1,0)  as push_totalamt,
                    decode((SUM(A09.totalamt) - SUM(A09.push_totalamt)),0,0,Round((SUM(A09.over_amt)-SUM(A09.push_over_amt))*10000/(SUM(A09.totalamt) - SUM(A09.push_totalamt)),0)) as over_total_rate
            from  (select * from cd01 where cd01.hsien_id <> 'Y') cd01
            left join wlx01 on wlx01.hsien_id=cd01.hsien_id
            left join bn01 on wlx01.bank_no=bn01.bank_no  and bn01.bank_type='6'
            left join (select * from A09
            ...
            */
            ...
            StringBuffer a09_table= new StringBuffer();//a09
            List a09_tableList =new ArrayList() ;
            a09_table.append(" (select * from ").append(cd01Table).append(" cd01 where cd01.hsien_id <> ?) cd01"
            ...
            a09_table.append("ALL".equals(bank_type)?" ? ,? ": "?");
            a09_table.append(") "
              	   				  + " left join (select * from A09 "
              	   				  + "  			where (to_char(m_year * 100 + m_month) >= ? and to_char(m_year * 100 + m_month) <= ?)");
            a09_tableList.add("Y") ;
            a09_tableList.add(u_year) ;
            a09_tableList.add(u_year) ;
            if("ALL".equals(bank_type)) {
            	a09_tableList.add("6") ;
            	a09_tableList.add("7") ;
            }else {
            	a09_tableList.add(bank_type) ;
            }
            a09_tableList.add(S_YEAR+S_MONTH) ;
            a09_tableList.add(E_YEAR+E_MONTH) ;
			if(hasBankListALL.equals("false")){
			   a09_table.append("         and  bank_code in ("+selectBank_no+")");
			}
            a09_table.append("           )A09 on bn01.bank_no = A09.bank_code");
            ...
            sqlCmd.append(" select bank_no,bank_name,"
            	   ...
            	   + " ) A09 ");
				   //+ " order by fr001w_output_order,bank_no,m_year,m_month";
			for(int a=0;a<a09_tableList.size() ;a++) {
				sqlCmdList.add(a09_tableList.get(a)) ;
			}
            a09_field_sum = " sum(A09.over_cnt)  as over_cnt,"
                          + " Round(sum(A09.over_amt)/1,0)  as over_amt,"
 						  + " Round(sum(A09.push_over_amt)/1,0)  as push_over_amt,"
 						  + " Round(sum(A09.totalamt)/1,0)  as totalamt,"
        				  + " Round(sum(A09.push_totalamt)/1,0)  as push_totalamt,"
			...
			for(int a=0;a<a09_tableList.size() ;a++) {
				sqlCmd_sumList.add(a09_tableList.get(a)) ;
			}
			if(!order.equals("")){
			    //各別機構
			...
			p.load(new FileInputStream(Utility.getProperties("schemaDir")+System.getProperty("file.separator")+"A09_ALL_column.TXT"));
			//====================================================================================================
            ...
            wb.setRepeatingRowsAndColumns(0, 1, columnLength+3, 1, 7); //設定表頭 為固定 先設欄的起始再設列的起始
  			...
  			//System.out.println("DS018W_Excel.sqlCmd="+sqlCmd);
  			//System.out.println("ori_field="+ori_field);
  			List dbData = null;
  			if(hasBankListALL.equals("false")){
			  dbData = DBManager.QueryDB_SQLParam(sqlCmd.toString(),sqlCmdList,"over_cnt,over_amt,push_over_amt,totalamt,push_totalamt,over_total_rate");
			  //System.out.println("DS018W_Excel.sqlCmd="+sqlCmd);
			}else{//95.09.04 add 金融機構代號=ALL全部
			  dbData = DBManager.QueryDB_SQLParam(sqlCmd_sum.toString(),sqlCmd_sumList,"over_cnt,over_amt,push_over_amt,totalamt,push_totalamt,over_total_rate");
			  //System.out.println("DS018W_Excel.sqlCmd_sum="+sqlCmd_sum);
			}
