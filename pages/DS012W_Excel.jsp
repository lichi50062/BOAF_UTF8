<%
// 95.11.3 create by 2295
// 95.11.3 農.漁會.都使用同一個設定檔 by 2295
// 95.12.06 add 增加年月區間,可根據年月做sort,拿掉可選欄位做sort by 2295
// 99.05.25 fixed by 2808 縣市合併&sql injection 
//102.12.31 add 本月平均利率/本年平均利率 by 2295
//103.01.15 add 縣市小計.增加本月平均利率(該縣市平均利率加總/已申報家數)/本年平均利率 by 2295
%>                                   
...
<%
	...
	try{
		...
		wlx01.append(" HSIEN_DIV_1,CANCEL_NO,CANCEL_DATE from wlx01 where m_year =? )");
			...
			prop_column.load(new FileInputStream(Utility.getProperties("schemaDir")+System.getProperty("file.separator")+"WLX07_M_CREDIT_detail.TXT"));
			//=======================================================================================================================
			...
			//String wlx07_m_credit_operation_field_sum="";
			//String wlx07_m_credit_operation_field="";
			StringBuffer wlx07_m_credit_field_sum= new StringBuffer();
			List wlx07sumList = new ArrayList() ;
			String ori_field="";
			String wlx07_m_creditacc_code="";
			//String wlx07_m_credit_field_sum_hsien_id = "";//縣市別用
			        ...
			        if(detail_column != null && detail_column.size() != 0){
			           columnLength += detail_column.size();//累加總欄位個數
              		   for(j=0;j<detail_column.size();j++){
            	 		   ...
            	 		   selectacc_code +="'"+wlx07_m_creditacc_code+"'";
            	 		   //95.09.26 add=================================================================================================================
            	 		   ori_field += wlx07_m_creditacc_code;
            	 		   //sum(d.ATM_CNT) as ATM_CNT,
            	 		   //if(wlx07_m_credit_field_sum_hsien_id.indexOf(" ,sum(d."+wlx07_m_creditacc_code+") as "+wlx07_m_creditacc_code) == -1){
            	 		   //    wlx07_m_credit_field_sum_hsien_id += " ,sum(d."+wlx07_m_creditacc_code+") as "+wlx07_m_creditacc_code;
            	 		   //}
            	 		   //====================================================================================================================================
			...
			prop_column_name.load(new FileInputStream(Utility.getProperties("schemaDir")+System.getProperty("file.separator")+"WLX07_M_CREDIT_column.TXT"));
			//====================================================================================================
            ...
            (
              select bn01.bank_no,bn01.bank_name,
                     ...
                     CreditMonth_Cnt,
                     Round(CreditMonth_Amt/1,0) as CreditMonth_Amt,
            	  	 CreditYear_Cnt_Acc,
            	  	 Round(CreditYear_Amt_Acc/1,0)  as CreditYear_Amt_Acc,
            	  	 Credit_Cnt,
            	  	 Round(Credit_Bal/1,0)  as Credit_Bal,
            	  	 OverCreditMonth_Cnt,
            	  	 Round(OverCreditMonth_Amt/1 ,0) as OverCreditMonth_Amt,
                	 OverCredit_Cnt,
            	  	 Round(OverCredit_Bal/1 ,0)  as  OverCredit_Bal
              ...
              left join (select * from WLX07_M_CREDIT
              	   		 where (to_char(m_year * 100 + m_month) >= 9501 and to_char(m_year * 100 + m_month) <= 9510) 
            			 and bank_no in ('6060019','6080066','6060156')
              	   		)WLX07_M_CREDIT on bn01.bank_no = WLX07_M_CREDIT.bank_no
              where WLX07_M_CREDIT.bank_no  <>  ' '  and  WLX07_M_CREDIT.CreditYear_Amt_Acc >= 0
              group by bn01.bank_no,bn01.bank_name,
                       ...
                       CreditMonth_Cnt,Round(CreditMonth_Amt/1,0),CreditYear_Cnt_Acc,Round(CreditYear_Amt_Acc/1,0),
            	  	   Credit_Cnt,Round(Credit_Bal/1,0),OverCreditMonth_Cnt,Round(OverCreditMonth_Amt/1 ,0),
                	   OverCredit_Cnt,Round(OverCredit_Bal/1 ,0)		
            ) WLX07_M_CREDIT
            order by fr001w_output_order,bank_no,m_year,m_month
            
            //縣市別.按縣市.年月sort(拿掉總計)
            select FR001W_output_order,hsien_id,hsien_name,m_yearmonth,m_year,m_month,cnt,CreditMonth_Cnt,CreditMonth_Amt,CreditYear_Cnt_Acc,
            	   CreditYear_Amt_Acc,Credit_Cnt,Credit_Bal,OverCreditMonth_Cnt,OverCreditMonth_Amt,OverCredit_Cnt,OverCredit_Bal,
				   decode(cnt,0,0,round(creditmonth_avgrate /  cnt  ,3))  as   creditmonth_avgrate,--103.01.15 add 本月平均利率/本年平均利率
				   decode(cnt,0,0,round(credityear_avgrate /  cnt  ,3))  as   credityear_avgrate
            from
            (
            select  cd01.FR001W_output_order as FR001W_output_order,
            		nvl(cd01.hsien_id,' ') as  hsien_id ,
                 	nvl(cd01.hsien_name,'OTHER')  as  hsien_name,
            		m_year || '/' || decode(substr(to_char(m_month),2),'','0','') || decode(substr(to_char(m_month),1),'1','1',substr(to_char(m_month),1)) as m_yearmonth,
            		m_year,m_month,  count(*) as cnt,      	
            		sum(WLX07_M_CREDIT.CreditMonth_Cnt)  as CreditMonth_Cnt,
            		Round(sum(WLX07_M_CREDIT.CreditMonth_Amt)/1,0)  as CreditMonth_Amt,
            		Sum(WLX07_M_CREDIT.CreditYear_Cnt_Acc) as CreditYear_Cnt_Acc,
            		Round(sum(WLX07_M_CREDIT.CreditYear_Amt_Acc)/1,0)  as CreditYear_Amt_Acc,
            		Round(sum(WLX07_M_CREDIT.Credit_Cnt)/1,0)  as Credit_Cnt,
                    Round(sum(WLX07_M_CREDIT.Credit_Bal)/1,0)  as Credit_Bal,
            		Sum(WLX07_M_CREDIT.OverCreditMonth_Cnt)  as OverCreditMonth_Cnt,
                    Round(sum(WLX07_M_CREDIT.OverCreditMonth_Amt)/1,0)   as  OverCreditMonth_Amt,
                	Sum(WLX07_M_CREDIT.OverCredit_Cnt)  as OverCredit_Cnt,
             		Round(sum(WLX07_M_CREDIT.OverCredit_Bal)/1,0)  as  OverCredit_Bal,
             		sum(creditmonth_avgrate) as creditmonth_avgrate ,
                    sum(credityear_avgrate) as credityear_avgrate
            from  (select * from cd01 where cd01.hsien_id <> 'Y') cd01
            left join wlx01 on wlx01.hsien_id=cd01.hsien_id
            left join bn01 on wlx01.bank_no=bn01.bank_no  and bn01.bank_type='6'
            left join (select * from WLX07_M_CREDIT 
            ...
            */            
            ...
            //add 營運中/已裁撤============================
            condition.append(" and bn01.bn_type "+(CANCEL_NO.equals("N")?"<>":"=")+" ? and wlx07_m_credit.bank_no = bn01.bank_no ");
            conditionList.add("2") ;
            //======================================================
            ...
            StringBuffer wlx07_m_credit_table= new StringBuffer();//wlx07_m_credit
            List wlx07List = new ArrayList();
            wlx07_m_credit_table.append(" (select * from ").append(cd01Table).append(" cd01 where cd01.hsien_id <> 'Y') cd01"
            ...
            wlx07_m_credit_table.append("ALL".equals(bank_type)? "? ,? " : " ?" );
            wlx07_m_credit_table.append(") "
              	   				  + " left join (select * from WLX07_M_CREDIT "
              	   				  + "  			where (to_char(m_year * 100 + m_month) >= ? and to_char(m_year * 100 + m_month) <= ? )");
            wlx07List.add(u_year)  ;
            wlx07List.add(u_year) ;
            if("ALL".equals(bank_type)) {
            	wlx07List.add("6") ;
            	wlx07List.add("7") ;
            }else {
            	wlx07List.add(bank_type) ;
            }
            wlx07List.add(S_YEAR+S_MONTH) ;
            wlx07List.add(E_YEAR+E_MONTH) ;
            
			if(hasBankListALL.equals("false")){	   				  
			   wlx07_m_credit_table.append("         and  bank_no in ("+selectBank_no+")");
			}	   				  
            wlx07_m_credit_table.append("           )WLX07_M_CREDIT on bn01.bank_no = WLX07_M_CREDIT.bank_no");
            
            sqlCmd.append(" select bank_no,bank_name,"
       			   + " 		  m_yearmonth,"
	   			   + "		  hsien_id,hsien_name,fr001w_output_order,"
			       + "		  creditmonth_cnt,creditmonth_amt,credityear_cnt_acc,credityear_amt_acc,credit_cnt,credit_bal,overcreditmonth_cnt,overcreditmonth_amt,overcredit_cnt,overcredit_bal,creditmonth_avgrate,credityear_avgrate"
            	   + " from "
            	   + " ( "
             	   + "	 select bn01.bank_no,bn01.bank_name,"
         		   + "			m_year || '/' || decode(substr(to_char(m_month),2),'','0','') || decode(substr(to_char(m_month),1),'1','1',substr(to_char(m_month),1)) as m_yearmonth,"
                   ...
                   + "		 	OverCredit_Cnt,"
            	   + "	 	 	Round(OverCredit_Bal/1 ,0)  as  OverCredit_Bal,"
            	   + "			creditmonth_avgrate," //102.12.31 add 本月平均利率/本年平均利率
            	   + "          credityear_avgrate "
              	   + "	 from " +wlx07_m_credit_table.toString()
              	   + " where WLX07_M_CREDIT.bank_no  <>  ' '  and  WLX07_M_CREDIT.CreditYear_Amt_Acc >= 0 "
              	   + " group by bn01.bank_no,bn01.bank_name,"
           		   + "		    m_year || '/' || decode(substr(to_char(m_month),2),'','0','') || decode(substr(to_char(m_month),1),'1','1',substr(to_char(m_month),1)),"
  		    	   + "			m_year,m_month,nvl(cd01.hsien_id,' '),nvl(cd01.hsien_name,'OTHER'),cd01.FR001W_output_order,"
           		   + "			CreditMonth_Cnt,Round(CreditMonth_Amt/1,0),CreditYear_Cnt_Acc,Round(CreditYear_Amt_Acc/1,0),"
	  	   		   + "			Credit_Cnt,Round(Credit_Bal/1,0),OverCreditMonth_Cnt,Round(OverCreditMonth_Amt/1 ,0),"
    	   		   + "			OverCredit_Cnt,Round(OverCredit_Bal/1 ,0),creditmonth_avgrate,credityear_avgrate"	
            	   + " ) WLX07_M_CREDIT ");
				   //+ " order by fr001w_output_order,bank_no,m_year,m_month";
            for(int t=0;t<wlx07List.size();t++) {
            	sqlCmdList.add(wlx07List.get(t)) ;
            }
            wlx07_m_credit_field_sum.append(" sum(WLX07_M_CREDIT.CreditMonth_Cnt)  as CreditMonth_Cnt,"
            		   				 + " Round(sum(WLX07_M_CREDIT.CreditMonth_Amt)/1,0)  as CreditMonth_Amt,"
            		   				 + " Sum(WLX07_M_CREDIT.CreditYear_Cnt_Acc) as CreditYear_Cnt_Acc,"
            		   				 + " Round(sum(WLX07_M_CREDIT.CreditYear_Amt_Acc)/1,0)  as CreditYear_Amt_Acc,"
            		   				 + " Round(sum(WLX07_M_CREDIT.Credit_Cnt)/1,0)  as Credit_Cnt,"
            	       				 ...
            	       				 + " Sum(WLX07_M_CREDIT.OverCredit_Cnt)  as OverCredit_Cnt,"
              		   				 ...
              		   				 + " sum(creditmonth_avgrate) as creditmonth_avgrate ,"
                    				 + " sum(credityear_avgrate) as credityear_avgrate ");
			sqlCmd_sum.append(" select FR001W_output_order,hsien_id,hsien_name,m_yearmonth,m_year,m_month,cnt,CreditMonth_Cnt,"
					   + " CreditMonth_Amt,CreditYear_Cnt_Acc,CreditYear_Amt_Acc, "
				       + " Credit_Cnt,Credit_Bal,OverCreditMonth_Cnt,OverCreditMonth_Amt,OverCredit_Cnt,OverCredit_Bal,"
					   + " decode(cnt,0,0,round(creditmonth_avgrate /  cnt  ,3))  as   creditmonth_avgrate,"
					   + " decode(cnt,0,0,round(credityear_avgrate /  cnt  ,3))  as   credityear_avgrate "
					   + " from ("
					   + " 	select cd01.FR001W_output_order as FR001W_output_order,"
					   + "		   nvl(cd01.hsien_id,' ') as  hsien_id ,"
                  	   ...
                  	   + "		   m_year || '/' || decode(substr(to_char(m_month),2),'','0','') || decode(substr(to_char(m_month),1),'1','1',substr(to_char(m_month),1)) as m_yearmonth,"
					   + "		   m_year,m_month, count(*) as cnt, "                   	   
             		   +           wlx07_m_credit_field_sum.toString()
              		   + "	from  " +wlx07_m_credit_table.toString()              		   
              		   + "  where m_year is not null"
					   + "  and   m_month is not null"
					   + "  group by cd01.FR001W_output_order,nvl(cd01.hsien_id,' '),nvl(cd01.hsien_name,'OTHER'),"
			...
			for(int t=0;t<wlx07List.size();t++) {
            	sqlCmd_sumList.add(wlx07List.get(t)) ;
            }
			if(!order.equals("")){
			    //各別機構
			...
			p.load(new FileInputStream(Utility.getProperties("schemaDir")+System.getProperty("file.separator")+"WLX07_M_CREDIT_column.TXT"));
			//====================================================================================================
            ...
            wb.setRepeatingRowsAndColumns(0, 1, columnLength+3, 1, 7); //設定表頭 為固定 先設欄的起始再設列的起始
  			...
  			//System.out.println("DS012W_Excel.sqlCmd="+sqlCmd);
  			//System.out.println("ori_field="+ori_field);
  			List dbData = null;
  			if(hasBankListALL.equals("false")){
			  //dbData = DBManager.QueryDB(sqlCmd,"creditmonth_cnt,creditmonth_amt,credityear_cnt_acc,credityear_amt_acc,credit_cnt,credit_bal,overcreditmonth_cnt,overcreditmonth_amt,overcredit_cnt,overcredit_bal");
			  dbData = DBManager.QueryDB_SQLParam(sqlCmd.toString(),sqlCmdList,"creditmonth_cnt,creditmonth_amt,credityear_cnt_acc,credityear_amt_acc,credit_cnt,credit_bal,overcreditmonth_cnt,overcreditmonth_amt,overcredit_cnt,overcredit_bal,creditmonth_avgrate,credityear_avgrate") ;
			  //System.out.println("DS012W_Excel.sqlCmd="+sqlCmd);
			}else{//95.09.04 add 金融機構代號=ALL全部
			  dbData = DBManager.QueryDB_SQLParam(sqlCmd_sum.toString(),sqlCmd_sumList,"creditmonth_cnt,creditmonth_amt,credityear_cnt_acc,credityear_amt_acc,credit_cnt,credit_bal,overcreditmonth_cnt,overcreditmonth_amt,overcredit_cnt,overcredit_bal,creditmonth_avgrate,credityear_avgrate");
			  //System.out.println("DS012W_Excel.sqlCmd_sum="+sqlCmd_sum);
			}
            ...
            for(i=0;i<dbData.size();i++){
                ...
                for(int cellIdx =4;cellIdx < (new Short(lastCellNum)).intValue();cellIdx++){
                     ...
                     if(acc_code.indexOf("avgrate") != -1){//102.12.31 add 本月平均利率/本年平均利率
                     	if(!amt.equals("")){
                     		amt = df_md.format(Double.valueOf(amt).doubleValue()).toString();                     	
                    	}
                     }else{	
                     	if(acc_code.indexOf("amt") != -1 || acc_code.indexOf("bal") != -1 ){//金額.餘額類
