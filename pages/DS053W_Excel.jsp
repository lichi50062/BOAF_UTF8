<%
//101.08.06 created 農信保--保證案件月報表(M201) by 2968
%>
	...
	try{	      	
			        ...
			        if(detail_column != null && detail_column.size() != 0){               
			           columnLength += detail_column.size();//累加總欄位個數
              		   for(j=0;j<detail_column.size();j++){
            	 		   ...
            	 		   if(!trans_field.equals("guarantee_cnt_month_0") && !trans_field.equals("guarantee_cnt_year_0") && !trans_field.equals("guarantee_cnt_sum_0")){
            	 		       if(field_sum.indexOf(trans_field) == -1){//95.10.03 fix 若不存在時,才加入sum的acc_code裡
            	 		          ...
            	 		          field_sum += " ,sum(decode(guarantee_item_no,'"+trans_field.substring(trans_field.length()-1,trans_field.length()).toUpperCase()+"',"+trans_field.substring(0,trans_field.length()-2)+",0)) as "+trans_field;
						       }              
            	 		   }            	 		  				
            	 		   //====================================================================================================================================
			...
			System.out.println("select acc_code="+selectacc_code);        		
  			...
  			select m_year || '/' || decode(length(m_month),'1','0','') || m_month as m_yearmonth,--資料年月
  				   guarantee_cnt_month_1,guarantee_cnt_year_1,guarantee_cnt_sum_1,--挑選欄位.政策性農業專案貸款-保證件數
                   guarantee_cnt_month_2,guarantee_cnt_year_2,guarantee_cnt_sum_2,--其他政策性貸款-保證件數
                   guarantee_cnt_month_0,guarantee_cnt_year_0,guarantee_cnt_sum_0--保證件數-合計
			from (
			 	 select m_year,m_month,
			       	    sum(decode(guarantee_item_no,'1',guarantee_cnt_month,0)) as guarantee_cnt_month_1,--本月份保證件數-政策性農業專案貸款
			            sum(decode(guarantee_item_no,'1',guarantee_cnt_year,0)) as guarantee_cnt_year_1,--本年度保證件數-政策性農業專案貸款
			            sum(decode(guarantee_item_no,'1',guarantee_cnt_sum,0)) as guarantee_cnt_sum_1,--累計保證件數-政策性農業專案貸款
			            sum(decode(guarantee_item_no,'2',guarantee_cnt_month,0)) as guarantee_cnt_month_2,--本月份保證件數-其他政策性貸款
			            sum(decode(guarantee_item_no,'2',guarantee_cnt_year,0)) as guarantee_cnt_year_2,--本年度保證件數-其他政策性貸款
			            sum(decode(guarantee_item_no,'2',guarantee_cnt_sum,0)) as guarantee_cnt_sum_2,--累計保證件數-其他政策性貸款
			            sum(decode(guarantee_item_no,'0',guarantee_cnt_month,0)) as guarantee_cnt_month_0,--本月份保證件數-合計
			            sum(decode(guarantee_item_no,'0',guarantee_cnt_year,0)) as guarantee_cnt_year_0,--本年度保證件數-合計
			            sum(decode(guarantee_item_no,'0',guarantee_cnt_sum,0)) as guarantee_cnt_sum_0--累計保證件數-合計
			      from m201
			      where (to_char(m_year * 100 + m_month) >= UI.查詢年度月份(起) ex:10001 and to_char(m_year * 100 + m_month) <= UI.查詢年度月份(迄) ex:10106)
			      group by m_year,m_month
			      )m201
			group by m_year,m_month,  
					 guarantee_cnt_month_1,guarantee_cnt_year_1,guarantee_cnt_sum_1,
			      	 guarantee_cnt_month_2,guarantee_cnt_year_2,guarantee_cnt_sum_2,
			         guarantee_cnt_month_0,guarantee_cnt_year_0,guarantee_cnt_sum_0
            ...
            sqlCmd.append(" sum(decode(guarantee_item_no,'0',guarantee_cnt_month,0)) as guarantee_cnt_month_0,");
            sqlCmd.append(" sum(decode(guarantee_item_no,'0',guarantee_cnt_year,0)) as guarantee_cnt_year_0,");
            sqlCmd.append(" sum(decode(guarantee_item_no,'0',guarantee_cnt_sum,0)) as guarantee_cnt_sum_0");
