...
//95.11.16 add 拿掉已選的報表欄位sort.法定比率報表有一定順序.只能根據bank_no排序 by 2295
...
<%
	...
	System.out.println("DS002W_RptOrder.szExcelAction="+szExcelAction);
...
if(session.getAttribute("btnFieldList") != null && !((String)session.getAttribute("btnFieldList")).equals("")){ 
   System.out.println("DS002W_RptOrder.btnFieldList="+(String)session.getAttribute("btnFieldList"));
%>
<%}%>
getbtnFieldList();
function getbtnFieldList(){
	...
	if(this.document.forms[0].BankList.value.indexOf("ALL") == -1){
       ...
       if(checkAdd == false){	  
          this.document.forms[0].SortListSrc.options[addCount] = new Option("金融機構代號名稱", "a02.bank_code");
          addCount++;
       }
    }else{//已選的機構代號=ALL全部
       /*95.11.16 拿掉年 sort
