/*
  Created on 2006/08/23 by 2495
  97.07.07 fix 即使為"0"也要顥示 by 2295
  98.01.17 fix 修正縣市政府,需列印農會+漁會的資料 by 2295
...
public class RptFR009WB {
  	public static String createRpt(String s_year,String s_month,String unit,String bank_type,List bank_list,String title_name)
	{
    System.out.println("RptFR009WB 開始!!");
		String errMsg = "";		
		String s_year_last="";
		String s_month_last="";	 
		...
		try{				
			...
			HSSFCellStyle cs2 = wb.createCellStyle();
        	 ...
        	 cell.setCellValue( title_name +"政府警示帳戶調查統計明細表");	
...
"        left join (select * from bn01 where m_year=?)bn01 on wlx01.bank_no=bn01.bank_no "+//98.01.17and bn01.bank_type='"+bank_type+"'        "+
"        left join (select * from WLX09_S_WARNING where m_year  = ? and m_quarter = ? ) a01    "+
" on  bn01.bank_no = a01.bank_no                                                                 "+
" ) a01   where a01.bank_no  <>  ' '"+//and  a01.WarnAccount_TCnt > 0 "+//97.07.07 fix 即使為"0"也要顥示
...
//==========================================================================================================		
     ...
     boolean isData = false;  //判斷選取的清單是否有筆數
     if(dbData!=null){
    ...
    for(int k=0;k<dbData.size();k++){
 		...
 		String hsien_id = (bean.getValue("hsien_id") == null)?"":String.valueOf(bean.getValue("hsien_id"));
 		String hsien_name = (bean.getValue("hsien_name") == null)?"":String.valueOf(bean.getValue("hsien_name")); 
 		String bank_no = (bean.getValue("bank_no") == null)?"":String.valueOf(bean.getValue("bank_no"));
 		String bank_name = (bean.getValue("bank_name") == null)?"":String.valueOf(bean.getValue("bank_name"));
 		String warnaccount_tcnt =  (bean.getValue("warnaccount_tcnt") == null)?"":Utility.setCommaFormat(String.valueOf(bean.getValue("warnaccount_tcnt")));
		String warnaccount_tbal =  (bean.getValue("warnaccount_tbal") == null)?"":Utility.setCommaFormat(String.valueOf(bean.getValue("warnaccount_tbal")));
		String warnaccount_remit_tcnt =  (bean.getValue("warnaccount_remit_tcnt") == null)?"":Utility.setCommaFormat(String.valueOf(bean.getValue("warnaccount_remit_tcnt")));
		String warnaccount_refund_apply_cnt =  (bean.getValue("warnaccount_refund_apply_cnt") == null)?"":(bean.getValue("hsien_id") == null)?"":Utility.setCommaFormat(String.valueOf(bean.getValue("warnaccount_refund_apply_cnt")));
		String warnaccount_refund_apply_amt =  (bean.getValue("warnaccount_refund_apply_amt") == null)?"":Utility.setCommaFormat(String.valueOf(bean.getValue("warnaccount_refund_apply_amt")));
		String warnaccount_refund_cnt =  (bean.getValue("warnaccount_refund_cnt") == null)?"":Utility.setCommaFormat(String.valueOf(bean.getValue("warnaccount_refund_cnt")));
		String warnaccount_refund_amt =  (bean.getValue("warnaccount_refund_amt") == null)?"":Utility.setCommaFormat(String.valueOf(bean.getValue("warnaccount_refund_amt")));
		...
		//System.out.println("warnaccount_tcnt= "+warnaccount_tcnt);
		//System.out.println("warnaccount_tbal= "+warnaccount_tbal);	
  ...
  if(!isData){
      		 ...
      		 cell = row.getCell( (short) 0);		
			 cell.setEncoding(HSSFCell.ENCODING_UTF_16);			
			 cell.setCellValue( s_year+" 年 "+s_month+" 季無資料 "+"                                單位："+unit_name+"，戶，筆");									   
