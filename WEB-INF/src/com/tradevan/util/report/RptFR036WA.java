...
public class RptFR036WA{                                      
  	public static String createRpt(String S_YEAR, String S_MONTH, String unit,String bank_type, List bank_list, String rptStyle) {
		...
		filename = "全體".concat(bankNm).concat("會信用部統一").concat(bankNm).concat("貸資料指定單月新增戶數金額比較").concat(reportType).concat(".xls");
		openfile = filename;
		System.out.println("rptStyle file " + openfile);
		...
		try {
			...
			FileInputStream finput = new FileInputStream(xlsDir
					+ System.getProperty("file.separator") + openfile);
			...
			// 中文才能正常顯示
			cell.setCellValue("全體"+bankNm+ "會信用部統一"+bankNm+"貸資料指定單月新增戶數金額比較 "+ reportType);
			...
			if (rptStyle.equals("0")) {  // 總表_各縣市==================================================================
				sql.append(getReportSql1(u_year)) ;
				paramList.add(unit) ;
				paramList.add(unit) ;
			...
			else if (rptStyle.equals("1") && bank_list.size() > 0) {
				if ("1".equals(rptStyle)) {
					sql.append(getReportSql2(u_year,bank_list,unit)) ;
			  		paramList.add(u_year) ;
			  		paramList.add(bank_type) ;
			  		paramList.add(u_year) ;
			  		paramList.add(S_YEAR) ;
			...
			System.out.print("單月總表抓出的dbData=" + dbData.size());
			DataObject bean = null;
			String unit_name = "";

			if (rptStyle.equals("1")) { //明細-------------------------------------------
				if (dbData.size() != 0) {
					System.out.println("列印年度-----------------------------------------");
				...
				if (dbData.size() != 0) {
					...
					for (int cellcount = 1; cellcount < 15; cellcount++) {
						...
						if (cellcount == 4 || cellcount == 7 || cellcount == 10|| cellcount == 13) {
							cell.setCellValue(S_YEAR_last + "年" + S_MONTH+ "月");
						}
			...
			}else  if(rptStyle.equals("0")){
				if (dbData.size() > 0) {
					System.out.println("start print 總表!!!!!!!!!!!!!!!");
				...
				if (dbData != null) {
					...
					for (int k = 0; k < dbData.size(); k++) {
						...
						String lastyearset_overcreditmonth_cnt = String.valueOf(bean.getValue("last_overcreditmonth_cnt"));
						String lastyearset_overcreditmonth_amt = String.valueOf(bean.getValue("last_overcreditmonth_amt"));
						System.out.print("Debug--lastyearset_overcreditmonth_amt = "+ lastyearset_overcreditmonth_amt);
				...
				// 列印年度
				System.out.println("dbData.size()=" + dbData.size());
				row = (sheet.getRow(1) == null) ? sheet.createRow(1) : sheet
						.getRow(1);
				...
				if (S_MONTH.equals("0")) {
					...
					cell.setCellValue("中華民國" + S_YEAR + "年" + S_MONTH
							+ "月無資料存在");
				}
			...
			FileOutputStream fout = new FileOutputStream(reportDir
					+ System.getProperty("file.separator") + openfile);
			wb.write(fout);
			// 儲存
			fout.close();
  	 ...
  	 * @return
  	 */
  	private static String getReportSql2(String u_year,List bank_list,String unit){
  		StringBuffer sql =new StringBuffer() ;
  		...
  		sql.append("              bn01.BANK_NAME                 as  bank_name,               ");
  		sql.append("              CreditMonth_Cnt                as  CreditMonth_Cnt,         ");
  		sql.append("              Round(CreditMonth_Amt/").append(unit).append(",0)     as  CreditMonth_Amt,         ");
  		sql.append("              OverCreditMonth_Cnt            as  OverCreditMonth_Cnt,     ");
  		sql.append("              Round(OverCreditMonth_Amt/").append(unit).append(",0) as  OverCreditMonth_Amt      ");
  		...
  		sql.append("       ) a01  ");
  		sql.append("       where a01.bank_no  <>  ' '  and a01.bank_no in (").append(getSelectBankNo(bank_list)).append(") and  a01.CreditMonth_Cnt >= 0                        ");
  		sql.append("       order by  a01.FR001W_output_order, a01.hsien_id,  a01.bank_no                                                             ");
  		sql.append("      ) PreYearSet                                                                                                               ");
  		sql.append("      left join                                                                                                                  ");
  		sql.append("      (select  * ");// --上個年度    
  		sql.append("       from (                                                                                                                    ");
  		sql.append("       select nvl(cd01.hsien_id,' ')         as  hsien_id ,                                                                      ");
  		sql.append("              nvl(cd01.hsien_name,'OTHER')   as  hsien_name,                                                                     ");
  		sql.append("              cd01.FR001W_output_order       as  FR001W_output_order,                                                            ");
    ...
    private static String getSelectBankNo(List bank_list) {
  	 ...
  	 * @return
  	 */
  	private static String getReportSql1(String u_year) {
  		StringBuffer sql = new StringBuffer();
  		if("100".equals(u_year)) {
	  		...
	  		sql.append(" 	   LastYearSet.OverCreditMonth_Amt	as	Last_OverCreditMonth_Amt    ");
	  		sql.append(" from                                                                 ");
	  		sql.append(" (select  * from (                                                    ");// --查詢年月
	  		sql.append("  select nvl(cd01.hsien_id,' ')       as  hsien_id ,                  ");
	  		sql.append("         nvl(cd01.hsien_name,'OTHER') as  hsien_name,                 ");
	  		sql.append("         cd01.FR001W_output_order     as  FR001W_output_order,        ");
	  		sql.append("         sum(CreditMonth_Cnt)                as  CreditMonth_Cnt,     ");
	  		sql.append("         Round(sum(CreditMonth_Amt)/?,0)     as  CreditMonth_Amt,     ");
	  		sql.append("         sum(OverCreditMonth_Cnt)            as  OverCreditMonth_Cnt, ");
	  		sql.append("         Round(sum(OverCreditMonth_Amt)/?,0) as  OverCreditMonth_Amt  ");
	  		...
	  		sql.append("                  and bn01.bank_type= ? and bn01.m_year = wlx01.m_year ");
	  		sql.append("                  and bn01.m_year = ? ");
	  		sql.append("  left join (select * from WLX07_M_CREDIT where m_year  =  ? and m_month =  ?) a01 on bn01.bank_no = a01.bank_no           ");
	  		sql.append("  group by nvl(cd01.hsien_id,' '),nvl(cd01.hsien_name,'OTHER'),cd01.FR001W_output_order                                     ");
	  		sql.append("  ) a01 order by  a01.FR001W_output_order, a01.hsien_id ) PreYearSet    ");
	  		sql.append("  left join ");
	  		sql.append(" (select  * from (  ");//--上個年度
	  		sql.append("  select nvl(cd01.hsien_id,' ')       as  hsien_id ,           ");
	  		sql.append("         nvl(cd01.hsien_name,'OTHER') as  hsien_name,          ");
	  		sql.append("         cd01.FR001W_output_order     as  FR001W_output_order, ");
	  		sql.append("         sum(CreditMonth_Cnt)                as  CreditMonth_Cnt,       ");
	  		...
	  		sql.append("                  and bn01.m_year = ? ");
	  		sql.append("  left join (select * from WLX07_M_CREDIT where m_year  = ? and m_month = ?) a01 on  bn01.bank_no = a01.bank_no             ");
	  		sql.append("  group by nvl(cd01.hsien_id,' '),nvl(cd01.hsien_name,'OTHER'),cd01.FR001W_output_order) a01  ");
	  		sql.append("  ) LastYearSet on PreYearSet.hsien_id=LastYearSet.hsien_id  ");
	  		sql.append("  union  ");
