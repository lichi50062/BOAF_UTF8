...
public class RptWR005W {
    ...
    public static String createRpt(String s_year,String s_month,String unit,HSSFWorkbook wb){
          ...
          String last_year  = String.valueOf(Integer.parseInt(s_year) - 1); 
          String last_month = s_month;
      try{
          ...
          logfile = new File(logDir + System.getProperty("file.separator") + "RptWR005W.log");
          
          System.out.println("logfile filename="+logDir + System.getProperty("file.separator") +"RptWR005W.log");
            ...
            String openfile="農漁會信用部營運狀況警訊報表-與上年度同期比較.xls";
            ...
            sqlCmd.append("  where wr_operation.m_year=? ");
	  		...
	  		sqlCmd.append("    and warn_type='Y' and wr_rpt='2' ");
            ...
            cell = row.getCell( (short)0);
            cell.setEncoding(HSSFCell.ENCODING_UTF_16);
            cell.setCellValue("與上一年度同期("+last_year+"年"+last_month+"月)比較");
    ...
    private static void getBeanData(DataObject bean){
    	try{
    	    ...
    	    diff_992710_990230_rate_max =bean.getValue("diff_992710_990230_rate_max") == null?"":Utility.setCommaFormat(String.valueOf(bean.getValue("diff_992710_990230_rate_max")));//本月份之(建築放款/上年度信用部決算淨值)-上月份之(建築放款/上年度信用部決算淨值)增加百分點前10名
    	    field_over_max =bean.getValue("field_over_max") == null?"":Utility.setCommaFormat(String.valueOf(bean.getValue("field_over_max")));//逾期放款本月前10名
    	    ...
    	    field_over_rate_max =bean.getValue("field_over_rate_max") == null?"":Utility.setCommaFormat(String.valueOf(bean.getValue("field_over_rate_max")));//逾放比率本月最高前10名
    	    field_diff_over_rate_max =bean.getValue("field_diff_over_rate_max") == null?"":Utility.setCommaFormat(String.valueOf(bean.getValue("field_diff_over_rate_max")));//本月份逾放比率-上月份逾放比率增加百分點前10名
    	    ...
    	    field_992710_990230_rate_avg =bean.getValue("field_992710_990230_rate_avg") == null?"":Utility.setCommaFormat(String.valueOf(bean.getValue("field_992710_990230_rate_avg")));//平均數.建築放款/上年度信用部決算淨值
    	    field_over_rate_avg =bean.getValue("field_over_rate_avg") == null?"":Utility.setCommaFormat(String.valueOf(bean.getValue("field_over_rate_avg")));//逾放比率.平均數
    	}catch(Exception e){
