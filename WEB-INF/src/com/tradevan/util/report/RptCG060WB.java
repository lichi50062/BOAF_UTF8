...
public class RptCG060WB {
	...
	public static String createRpt(String startYear,String startMonth, String endYear, String endMonth, String bankType) {
		...
		if(debug) System.out.println("RptCG060WB createRpt() Debug Start ...");
		String errMsg = "";
		String sqlCmd = "";
...
//==== 先查詢出所有機關名稱
			...
			HashMap bankUserMap = new HashMap();//裡面以Bank_no當Key,裡面放List
...
//=== 依年月查詢
			for(int mi=0;mi<monthIndex;mi++){
				...
				if(queryDate.after(calendar)){
					sqlCmd="SELECT w.TBANK_NO, w.MUSER_ID, w.MUSER_NAME, COUNT(a.SERIALNO) NUMS FROM WTT06 a, WTT01 w"
						+" WHERE w.MUSER_ID=a.MUSER_ID AND TO_CHAR(a.INPUT_DATE, 'yyyy/mm')  = ?";
					if(!bankType.equals("ALL")){
						sqlCmd+=" AND w.BANK_TYPE=?";
					}
					sqlCmd+=" GROUP BY w.TBANK_NO, w.MUSER_ID, w.MUSER_NAME ORDER BY w.TBANK_NO, w.MUSER_NAME";
					if(debug) System.out.println("sqlCmd="+sqlCmd);
					pst = conn.prepareStatement(sqlCmd);
					...
					while(rs.next()){
						...
						if(bankNoTmp.trim().length()==0)
							bankNoTmp="empty";
						String userIdTmp = rs.getString(2)==null?"":rs.getString(2);
						String userNameTmp = rs.getString(3)==null?"":rs.getString(3);
						userList = (List)bankUserMap.get(bankNoTmp);
						if(userList==null){
							userList = new ArrayList();
						}
						boolean addToUserList = true;
						for(int li=0; li<userList.size(); li++){
							String[] userData = (String[])userList.get(li);
							if(userData[0].equals(userIdTmp)){
								addToUserList=false;
								break;
							}
						}
						if(addToUserList){
							userList.add(new String[]{userIdTmp, userNameTmp});
						}
						String nums = rs.getString(4);
						if(resultMap.get(bankNoTmp+"_"+queryYear+queryMonth)!=null){
							dataValueTmp = (String[])resultMap.get(bankNoTmp+"_"+queryYear+queryMonth+"@"+userIdTmp);
						}
						dataValueTmp[0]=nums;
						dataValueTmp[1]="";
						if(debug) System.out.println("加到map內: key="+bankNoTmp+"_"+queryYear+queryMonth+"@"+userIdTmp);
						resultMap.put(bankNoTmp+"_"+queryYear+queryMonth+"@"+userIdTmp, dataValueTmp);
						bankUserMap.put(bankNoTmp, userList);
					}
                                        ...
                                        if(nowDate.equals(queryYear + "/" + queryMonth)){
                                          sqlCmd="SELECT w.TBANK_NO, w.MUSER_ID, COUNT(a.SERIALNO) NUMS FROM WTT06 a, WTT01 w"
                                                +" WHERE w.MUSER_ID=a.MUSER_ID AND a.INPUT_DATE BETWEEN to_date(?, 'yyyy/mm') AND to_date(?, 'yyyy/mm')";
                                          if (!bankType.equals("ALL")) {
                                              sqlCmd += " AND w.BANK_TYPE=?";
                                          }
                                          sqlCmd+=" GROUP BY w.TBANK_NO, w.MUSER_ID ORDER BY w.TBANK_NO, w.MUSER_ID";
                                          if(debug) System.out.println("sqlCmd="+sqlCmd);
                                          pst = conn.prepareStatement(sqlCmd);
                                          pst.setString(1, startQueryDate);
                                          ...
                                          rs = pst.executeQuery();
                                          while(rs.next()){
                                              threeMonthMap.put(rs.getString(1)+"_"+queryYear+queryMonth+"@"+rs.getString(2),rs.getString(3));
                                          }
                                        }else{
                                          sqlCmd="SELECT w.TBANK_NO, w.MUSER_ID, COUNT(a.SERIALNO) NUMS FROM WTT06 a, WTT01 w"
                                                +" WHERE w.MUSER_ID=a.MUSER_ID AND a.INPUT_DATE BETWEEN to_date(?, 'yyyy/mm') AND to_date(?, 'yyyy/mm')";
					...
					sqlCmd="SELECT BANK_NO, LOGIN_NUM, USER_ID, USER_NAME "
                                            + "FROM STATISTICS_BAK WHERE TB_NAME=? AND TO_NUMBER(M_YEAR+1911)=? AND M_MONTH=?";
					if(!bankType.equals("ALL")){
						sqlCmd+=" AND BANK_TYPE=?";
					}
					...
					while(rs.next()){
						...
						String userIdTmp = rs.getString(3);
						String userNameTmp = rs.getString(4);
						userList = (List)bankUserMap.get(bankNoTmp);
						if(userList==null){
							userList = new ArrayList();
						}
					...
					sqlCmd="SELECT BANK_NO, LOGIN_NUM, USER_ID FROM STATISTICS_BAK WHERE TB_NAME=?";// AND M_YEAR=? AND M_MONTH=?";
					...
					sqlCmd+=" ORDER BY BANK_NO, USER_ID";
					if(debug) System.out.println("sqlCmd="+sqlCmd);
					pst = conn.prepareStatement(sqlCmd);
					pst.setString(1, "WTT06");
					Calendar befor3MDate = Calendar.getInstance();
					...
					while(rs.next()){
						...
						String userId2 = rs.getString(3);
						if(threeMonthMap.get(bankNo2+"_"+queryYear+queryMonth+"@"+userId2)!=null){
							lginNums = Integer.parseInt(lginNums)+ Integer.parseInt((String)threeMonthMap.get(bankNo2+"_"+queryYear+queryMonth+"@"+userId2))+"";
						}
						threeMonthMap.put(bankNo2+"_"+queryYear+queryMonth+"@"+userId2,rs.getString(2));
					}
				}
			}
			//設定報表表頭資料============================================
			HSSFWorkbook wb = new HSSFWorkbook();//new HSSFWorkbook(fs);
			...
			if (bankList.size()==0) {
				...
				for(int li=0;li<bankList.size(); li++){
					...
					List bankUserList = bankUserMap.get(bankNoTmp)==null?new ArrayList():(List)bankUserMap.get(bankNoTmp);
					int[] totalNums = new int[monthIndex];
					if(debug) System.out.println("bankUserList size="+bankUserList.size());
					for(int ui=0; ui<bankUserList.size(); ui++){
						String[] userData = (String[])bankUserList.get(ui);
						for(int mi=0;mi<monthIndex;mi++){
							if((Integer.parseInt(startMonth)+mi)>12){
								queryYear = String.valueOf(Integer.parseInt(startYear)+1);
								queryMonth=(Integer.parseInt(startMonth)+mi-12)+"";
							...
							//====列印報表資料
							String dataValueTmp[] = new String[2];
							if(debug) System.out.println("get value from map: Key="+bankNoTmp+"_"+queryYear+queryMonth+"@"+userData[0]+"; value="+resultMap.get(bankNoTmp+"_"+queryYear+queryMonth));
							if(resultMap.get(bankNoTmp+"_"+queryYear+queryMonth+"@"+userData[0])!=null){
								dataValueTmp = (String[])resultMap.get(bankNoTmp+"_"+queryYear+queryMonth+"@"+userData[0]);
								totalNums[mi]+=Integer.parseInt(dataValueTmp[0]);
							}else{
								dataValueTmp[0]="0";
								dataValueTmp[1]="N";
							}
							...
							if(dataValueTmp[0].equals("0") || dataValueTmp[1].equals("N")){
								if(threeMonthMap.get(bankNoTmp+"_"+queryYear+queryMonth+"@"+userData[0])!=null){
						...
						if(ui==(bankUserList.size()-1)){
							row=sheet.createRow(rowNo++);
							reportUtil.createCell(wb, row, (short)0, "合計", rightStyle);
							for(int mi=0;mi<monthIndex;mi++){
								int cellNo2=1+(2*mi);
								reportUtil.createCell(wb, row, (short)cellNo2, String.valueOf(totalNums[mi]), rightStyle);
								reportUtil.createCell(wb, row, (short)(cellNo2+1), "", defaultStyle);
							}
						}
					}
				}//end of for
			...
			FileOutputStream fout = new FileOutputStream(reportDir + System.getProperty("file.separator") + "使用者帳號期間未使用明細表.xls");
			...
			System.out.println("//RptCG060WB createRpt() Have Error.....");
			e.printStackTrace();
			System.out.println("//-------------------------------------");
		}finally{
			try{
		...
		if(debug) System.out.println("RptCG060WB createRpt() Debug End ...");
		return errMsg;
	}
}
