/*
	...
	94.10.27 重新增加一張表格並增加部份欄位(存款總額220000)fieldD1  及fieldD2[農會A(放款總額)=120000 + 120800 + 150300][漁會A(放款總額)=120000 + 120800 + 121000+ 150300])  BY  蔡世榮
	94.10.28 FR001WA報表新增完成 by 2295
	99.09.17 fix 根據查詢年度.100年以後取得新縣市別.100年以前取得舊縣市別
  			      使用PreparedStatement;並列印轉換後的SQL;套用QueryDB_SQLParam by 2295
   100.10.13 fix 101年.顯示更改後的組織名稱(行政院農業委員會->行政院農業部) by 2295 
   102.06.21 fix SQL調整  by 2968 
...
public class RptFR001WA {
    public static String createRpt(String s_year,String s_month,String unit,String datestate,String bank_type,String rptStyle){
		...
		String s_year_last="";
		String s_month_last="";
		String hsien_id_sum[]={"","b","c","d","d1","d2","e","f","g","h","i","j","k","l","m","n","o","p"};
		String bank_type_name=(bank_type.equals("6"))?"農會":"漁會";
		String filename="";
		filename=(bank_type.equals("6"))?"台閩地區農會信用部經營指標_A.xls":"台閩地區漁會信用部經營指標_A.xls";
		...
		try{
    		...
    		String openfile="台閩地區農漁會信用部經營指標_A"+(rptStyle.equals("0")?((Integer.parseInt(s_year) <= 99)?"_99":(bank_type.equals("6"))?"_農會":"_漁會"):"_明細表")+".xls";
	        ...
	        ps.setPaperSize( ( short )9 ); //設定紙張大小 A4
	  		...
	  		sqlCmd.append("         round(sum(a01.fieldD1)/?,0) as fieldD1 ,");//--存款總額-當月底不需調整
	  		sqlCmd.append("         round(sum(a01.fieldD2)/?,0) as fieldD2 ,");//--放款總額-當月底不需調整 
	  		...
	  		sqlCmd.append(" left join ( select bank_code, sum(fieldB) as fieldB ,   sum(fieldC) as fieldC ,        sum(fieldD) as fieldD , sum(fieldD1) as fieldD1 ,");
	  		sqlCmd.append("                    sum(fieldD2) as fieldD2 ,  sum(fieldE) as fieldE ,          sum(fieldF) as fieldF , sum(fieldG) as fieldG ,");          
	  		sqlCmd.append("                    sum(fieldH) as fieldH , sum(fieldI) as fieldI ,         sum(fieldJ) as fieldJ , sum(fieldK) as fieldK ,");          
	  		sqlCmd.append("                    sum(fieldL) as fieldL , sum(fieldM) as fieldM ,         sum(fieldN) as fieldN , sum(fieldO) as fieldO ,");          
	  		sqlCmd.append("                    sum(fieldP) as fieldP , sum(field_120000) as field_120000 ");    
	  		sqlCmd.append("             from (  (select bank_code, fieldB, fieldC,0 as fieldD,");//--存款總額/放款總額,查詢年月及前一月份加總             
	  		sqlCmd.append("                             0 as fieldD1, 0 as fieldD2, 0 as fieldE, 0 as fieldF, 0 as fieldG, 0 as fieldH, 0 as fieldI,");          
            ...
            paramList.add(wlx01_m_year);
	  		...
	  		sqlCmd.append("                            where ((a01.m_year = ? and a01.m_month= ?) or (a01.m_year = ? and a01.m_month= ?)) ");
            ...
            paramList.add(s_month_last);
	  		...
	  		sqlCmd.append("                      (SELECT bank_code,  0  as fieldB, 0  as fieldC,  0  as fieldD,");          
	  		sqlCmd.append("                              sum(decode(a01.acc_code,'220000',amt,0)) AS fieldD1,");            
	  		sqlCmd.append("                              decode(bank_type,'6',sum(decode(a01.acc_code,'120000',amt,'120800',amt,'150300',amt,0)),");                             
	  		sqlCmd.append("                              '7',sum(decode(a01.acc_code,'120000',amt,'120800',amt,'121000',amt,'150300',amt,0)))  AS fieldD2,");           
	  		sqlCmd.append("                              sum(decode(a01.acc_code,'990000',amt,0)) AS fieldE,");         
	  		sqlCmd.append("                              round(decode(sum(decode(a01.acc_code,'120000',amt,0)),0,0,");          
	  		sqlCmd.append("                              sum(decode(a01.acc_code,'990000',amt,0)) /sum(decode(a01.acc_code,'120000',amt,0))),4) AS fieldF,");           
	  		sqlCmd.append("                              sum(decode(a01.acc_code,'150200',amt,0)) AS fieldG,");         
	  		sqlCmd.append("                              sum(decode(a01.acc_code,'120800',amt,'150300',amt,0)) AS fieldH,");            
	  		...
	  		sqlCmd.append("                              decode(bank_type,'6',sum(decode(a01.acc_code,'120000',amt,'120800',amt,'150300',amt,0)),'7',sum(decode(a01.acc_code,'120000',amt,'120800',amt,'121000',amt,'150300',amt,0)))  AS field_120000 ");  
	  		sqlCmd.append("                       from a01,(select ba01.bank_type,wlx01.hsien_id,ba01.bank_no ");             
	  		sqlCmd.append("                       from (select * from ba01 where m_year=? and bank_type=?)ba01,(select * from wlx01 where m_year=?)wlx01 ");              
            ...
            paramList.add(s_month);
	  		sqlCmd.append("                       and   (a01.bank_code=ba01.bank_no) "); 
	  		sqlCmd.append("                       group by ba01.bank_type,a01.bank_code ");
	  		sqlCmd.append("                     ) ");
	  		sqlCmd.append("                  )  a01 ");
	  		sqlCmd.append("          group by a01.bank_code ");
            ...
            sqlCmd_each.append(" order by cd01.FR001W_output_order ");
	  		...
	  		sqlCmd_detail.append("         round(sum(a01.fieldD1)/ ?,0) as fieldD1 , ");//--存款總額-當月底不需調整 
	  		sqlCmd_detail.append("         round(sum(a01.fieldD2)/ ?,0) as fieldD2 , ");//--放款總額-當月底不需調整
            ...
            paramList_detail.add(wlx01_m_year);
	  		...
	  		sqlCmd_detail.append("      ( select bank_code, sum(fieldB) as fieldB ,   sum(fieldC) as fieldC , ");        
	  		sqlCmd_detail.append("               sum(fieldD) as fieldD , sum(fieldD1) as fieldD1 ,sum(fieldD2) as fieldD2 , ");
	  		sqlCmd_detail.append("               sum(fieldE) as fieldE , sum(fieldF) as fieldF , sum(fieldG) as fieldG , ");
	  		sqlCmd_detail.append("               sum(fieldH) as fieldH , sum(fieldI) as fieldI , sum(fieldJ) as fieldJ , "); 
	  		sqlCmd_detail.append("               sum(fieldK) as fieldK , sum(fieldL) as fieldL , sum(fieldM) as fieldM , ");           
	  		sqlCmd_detail.append("               sum(fieldN) as fieldN , sum(fieldO) as fieldO , sum(fieldP) as fieldP , "); 
	  		sqlCmd_detail.append("               sum(field_120000) as field_120000 ");    
	  		sqlCmd_detail.append("      from (  ");
	  		sqlCmd_detail.append("            ( select bank_code, fieldB, fieldC,  0 as fieldD, ");             
	  		sqlCmd_detail.append("                     0 as fieldD1, 0 as fieldD2, 0 as fieldE, 0 as fieldF, 0 as fieldG, 0 as fieldH, 0 as fieldI, ");             
            ...
            paramList_detail.add(wlx01_m_year);
	  		sqlCmd_detail.append("                                      where ba01.bank_no = wlx01.bank_no)ba01 ");          
	  		sqlCmd_detail.append("                            where ((a01.m_year = ? and a01.m_month= ? ) or (a01.m_year = ? and a01.m_month= ? )) "); 
            ...
            paramList_detail.add(s_month_last);
	  		...
	  		sqlCmd_detail.append("            (SELECT bank_code,  0  as fieldB, 0  as fieldC,  0  as fieldD, ");            
	  		sqlCmd_detail.append("                    sum(decode(a01.acc_code,'220000',amt,0)) AS fieldD1, ");            
	  		sqlCmd_detail.append("                    decode(bank_type,'6',sum(decode(a01.acc_code,'120000',amt,'120800',amt,'150300',amt,0)), '7',sum(decode(a01.acc_code,'120000',amt,'120800',amt,'121000',amt,'150300',amt,0)))  AS fieldD2, ");            
	  		sqlCmd_detail.append("                    sum(decode(a01.acc_code,'990000',amt,0)) AS fieldE, ");            
	  		...
	  		sqlCmd_detail.append("                    sum(decode(a01.acc_code,'150200',amt,0))      AS fieldG, ");            
	  		sqlCmd_detail.append("                    sum(decode(a01.acc_code,'120800',amt,'150300',amt,0)) AS fieldH, ");
	  		...
	  		sqlCmd_detail.append("                    decode(bank_type,'6',sum(decode(a01.acc_code,'120000',amt,'120800',amt,'150300',amt,0)),'7',sum(decode(a01.acc_code,'120000',amt,'120800',amt,'121000',amt,'150300',amt,0)))  AS field_120000 ");     
	  		sqlCmd_detail.append("            from a01,(select ba01.bank_type,wlx01.hsien_id,ba01.bank_no ");               
	  		sqlCmd_detail.append("            from (select * from ba01 where m_year=? and bank_type=?)ba01,(select * from wlx01 where m_year=?)wlx01 ");              
            ...
            paramList_detail.add(s_month);
	  		sqlCmd_detail.append("            and  (a01.bank_code=ba01.bank_no) "); 
	  		sqlCmd_detail.append("            group by ba01.bank_type,a01.bank_code ");
	  		sqlCmd_detail.append("           ) "); 
	  		sqlCmd_detail.append("         )  a01 group by a01.bank_code "); 
	  		sqlCmd_detail.append("    ) a01 ");   
	  		sqlCmd_detail.append(" where  ba01.hsien_id=cd01.hsien_id ");  
	  		sqlCmd_detail.append(" and    ba01.bank_no=a01.bank_code  "); 
	  		sqlCmd_detail.append(" group by nvl(cd01.hsien_id,' '),nvl(cd01.hsien_name,'其他'),cd01.FR001W_output_order,ba01.bank_type,ba01.bank_no ,ba01.BANK_NAME "); 
	  		sqlCmd_detail.append(" order by cd01.FR001W_output_order,  ba01.bank_no ");

			if(rptStyle.equals("0")){//總表
			   dbData_sum = DBManager.QueryDB_SQLParam(sqlCmd_sum.toString(),paramList,"fieldb,fieldc,fieldd, fieldd1, fieldd2, fielde,fieldf,fieldg,fieldh,fieldi,fieldj,fieldk,fieldl,fieldm,fieldn,fieldo,fieldp");
			   System.out.println("總計的dbData_sum.size()="+dbData_sum.size());
			   dbData_sumtaiwan = DBManager.QueryDB_SQLParam(sqlCmd_sumtaiwan.toString(),paramList,"fieldb,fieldc,fieldd, fieldd1, fieldd2, fielde,fieldf,fieldg,fieldh,fieldi,fieldj,fieldk,fieldl,fieldm,fieldn,fieldo,fieldp");
			   System.out.println("台灣省小計的dbData_sumtaiwan.size()="+dbData_sumtaiwan.size());
			   dbData_sumfuchien = DBManager.QueryDB_SQLParam(sqlCmd_sumfuchien.toString(),paramList,"fieldb,fieldc,fieldd, fieldd1, fieldd2, fielde,fieldf,fieldg,fieldh,fieldi,fieldj,fieldk,fieldl,fieldm,fieldn,fieldo,fieldp");
			   System.out.println("福建省小計的dbData_sumfuchien.size()="+dbData_sumfuchien.size());
			   dbData_each = DBManager.QueryDB_SQLParam(sqlCmd_each.toString(),paramList,"fieldb,fieldc,fieldd, fieldd1, fieldd2, fielde,fieldf,fieldg,fieldh,fieldi,fieldj,fieldk,fieldl,fieldm,fieldn,fieldo,fieldp");
			   System.out.println("各縣市小計的dbData_each.size()="+dbData_each.size());
			}else{//明細表
			   dbData_detail = DBManager.QueryDB_SQLParam(sqlCmd_detail.toString(),paramList_detail,"fieldb,fieldc,fieldd, fieldd1, fieldd2, fielde,fieldf,fieldg,fieldh,fieldi,fieldj,fieldk,fieldl,fieldm,fieldn,fieldo,fieldp");
			...
			if(rptStyle.equals("0")){//總表
			    ...
			    */
				...
				insertBlank(17,wb,(short)0,(short)0,row);
