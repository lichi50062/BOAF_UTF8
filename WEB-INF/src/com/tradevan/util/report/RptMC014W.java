...
public class RptMC014W {	 
	 public static String createRpt(String bank_no) {    
	    ...
	    try {
	      ...
	      cell.setCellValue(tbank_no_name+"檢舉書");
	      ...
	      {	          
	    	  rowNum = 2;//從表格的第二列開始放置第一筆資料(因編號0~1皆為表頭資料)
	    	    ...
	    	    ta_ps=(bean.getValue("ta_ps") == null)?"":(String)bean.getValue("ta_ps");
				   ...
				   for(int cellcount=0;cellcount<=5;cellcount++){			 	      
			    	   ...
			    	   { 
			    		   cell.setCellStyle(cs_center);
			    		   cell.setCellValue(ta_number);//收文文號
			    	   }
			    	   else if(cellcount == 1)
			    	   { 
			    		   cell.setCellStyle(cs_center);
			    		   cell.setCellValue(ta_date);//收文日期	
			    	   }
			    	   else if(cellcount == 2)
			    	   { 
			    		   cell.setCellStyle(cs_center);
			    		   cell.setCellValue(ta_reporter);//檢舉人	
			    	   }
			    	   else if(cellcount == 3)
			    	   { 
			    		   cell.setCellStyle(cs_center);
			    		   cell.setCellValue(bank_name);//被檢舉機構
			    	   }
			    	   else if(cellcount == 4)
			    	   { 
			    		   cell.setCellStyle(cs_left);
			    		   cell.setCellValue(ta_content);//檢舉內容
			    	   }
			    	   else if(cellcount == 5)
			    	   { 
			    		   cell.setCellStyle(cs_left);
			    		   cell.setCellValue(ta_ps);//備註
			    	   }
	      	...
	      	cell.setCellValue(tbank_no_name+"檢舉書無資料");
	      }
	      ...
	      System.out.println("RptMC014W.createRpt Error:" + e + e.getMessage());
	    }
