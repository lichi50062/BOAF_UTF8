...
//			Creating Cells
            ...
            createCell( wb, row, ( short )5, "總機構代號/名稱", defaultStyle );
            createCell( wb, row, ( short )6, "分支機構代號/名稱", defaultStyle );
            createCell( wb, row, ( short )7, "異動職位/姓名", defaultStyle );
            createCell( wb, row, ( short )8, "操作類別", defaultStyle );
            ...
            if(bodyList.size()>0){
                for(int i=0; i < bodyList.size(); i++) {
                    ...
                    String pbank_no = bean.getValue("pbank_no") != null ? (String)bean.getValue("pbank_no")+"/" : "" ;
                    String bank_no = bean.getValue("bank_no") != null ? (String)bean.getValue("bank_no")+"/" : "" ; 
                    String pbank_name = bean.getValue("pbank_name") != null ? (String)bean.getValue("pbank_name") : "" ;
                    String bank_name = bean.getValue("bank_name") != null ? (String)bean.getValue("bank_name") : "" ;
                    String position_name = bean.getValue("position_name") != null ? (String)bean.getValue("position_name")+"/" : "" ; 
                    String upd_name = bean.getValue("upd_name") != null ? (String)bean.getValue("upd_name") : "" ;
                    ...
                    createCell( wb, row, ( short )5, pbank_no + pbank_name, defaultStyle );
                    createCell( wb, row, ( short )6, bank_no  + bank_name, defaultStyle );
                    createCell( wb, row, ( short )7, position_name + upd_name, defaultStyle );
                    createCell( wb, row, ( short )8, update_type, defaultStyle );
                	rowNo++;
...
//          設定涷結欄位
            ...
            fileOut = new FileOutputStream(reportDir + System.getProperty("file.separator")+"ZZ093W.xls");
            wb.write( fileOut );
            fileOut.close();
			...
			System.out.println("ZZ093W_report:Exception  action=create_File  msg=" + e.getMessage());
			e.printStackTrace();
			fileName = null;
		} finally {
			try {
				...
				System.out.println("ZZ093W_report:Exception  action=close_File  msg=" + e.getMessage());
			}
		}
