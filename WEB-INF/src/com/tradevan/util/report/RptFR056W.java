/*
99.08.06 add 信用部主任及分部主任參訓情形總表 by 2660
...
public class RptFR056W {
	...
	public static String createRpt(String s_year, String s_month, String bank_type)	{
		...
		try {
			...
			String openfile = "信用部主任及分部主任參訓情形總表.xls";	
			...
			sql.append("       	  wlx_trainning.wlx02count,total_hour,person_mm, hour_mm, person_yy,hour_yy, less_hour ");
			...
			sql.append("                      total_hour,");//本年至少應進修時數合計  
			sql.append("                      person_mm, ");//本月人次
			sql.append("                      hour_mm, ");//本月時數
			sql.append("                      person_yy,");//本年人次
			sql.append("                      hour_yy,");//本年時數
			sql.append("                      decode(hour_yy,null,0,hour_yy) - total_hour AS less_hour");//進修不足時數
			sql.append("               from"); 								   
			...
			paramList.add(wlx01_m_year);
			sql.append("                            AND cancel_no <> 'Y' OR cancel_no IS NULL");
			...
			sql.append("                                  WHERE wlx_trainning.m_year = ? AND wlx_trainning.m_month = ?");
			sql.append("                                  AND position_code IN ('1', '2')");
			...
			List dbData = DBManager.QueryDB_SQLParam(sql.toString(),paramList, "wlx02count,total_hour,person_mm,hour_mm,person_yy,hour_yy,less_hour");
			...
			sql.append("         sum(total_hour) as total_hour,");
			...
			sql.append(" order by  Temp1.FR001W_output_order, Temp1.bank_no ");
			sql.append(" )");
			List dbData_sum = DBManager.QueryDB_SQLParam(sql.toString(),paramList, "bank_name,wlx02count,total_hour,person_mm,hour_mm,person_yy,hour_yy,less_hour");
			...
			if(dbData.size() == 0){
				...
				for(int tablecount=0;tablecount <2;tablecount++){
					if(tablecount == 0) dbData_count = dbData;//列印明細資料
				    ...
				    for (int rowcount = 0; rowcount < dbData_count.size(); rowcount++) {
				    	...
				    	for (int cellcount = 0; cellcount < 9; cellcount++) {
				    		if (cellcount == 0)
				    			...
				    			sValue = (bean.getValue("total_hour") == null)?"":bean.getValue("total_hour").toString();
				    		else if (cellcount == 4)
				    			...
				    			if(tablecount == 0) sValue = (bean.getValue("less_hour") == null)?"":bean.getValue("less_hour").toString();
				    			if(tablecount == 1) sValue = "";
				    		}	
				    		...
				    		if (cellcount == 8 && !sValue.equals("")) {						   
				    			cell.setCellValue((Integer.parseInt(sValue) < 0)?"＊":"");
				    		}else if (cellcount == 0 || cellcount == 1){							
			...
			//cs1.setAlignment(HSSFCellStyle.ALIGN_LEFT);
			cell.setCellStyle(cs1);
			cell.setCellValue("未達至少應進修時數之金融機構以＊表示。");
