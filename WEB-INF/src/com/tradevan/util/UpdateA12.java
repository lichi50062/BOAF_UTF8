//104.01.13 copyA08&fix by 2968 
//104.02.11 fix 調整A12檢核邏輯 by 2295
...
public class UpdateA12 {
	private static String errMsg = "";
	public String getErrMsg(){
		return errMsg;  
	}
	public static synchronized String doParserReport_A12(String report_no, String m_year,
		...
		try { 
			...
			if(input_method.equals("F")){//若為檔案上傳,先將檔案讀取出來
				List allList = getA12FileData(report_no,filename,m_year,m_month);						
			...
			Utility.printLogTime("A12-1 time");	
			//若為"Y"表示由共用中心代傳入			
			...
			Utility.printLogTime("A12-2 time");	
			...
			if(input_method.equals("F")){//在A12中無資料的先將insert Zero data 到A12
		    	errMsg += Utility.InsertZeroAXX_List(m_year,m_month,bank_codeList,report_no,"");//在A12中無資料的先將insert Zero data 到A12
		    	Utility.InsertAXX_LOG(m_year,m_month,user_id,user_name,filename,report_no);//批次寫入(有異動)至A01_LOG		    			    	
		    	//99.11.11 A12沒有科目代號,不需檢核科目代號存不存在errMsg += Utility.InsertWML03(m_year,m_month,user_id,user_name,report_no,filename);//農漁會.批次寫入檢核其他錯誤(科目代號不存在)		    	
		    	errMsg += Utility.updateAXX(m_year,m_month,report_no,filename);//將有異動的資料update A01
			}//end of 檔案上傳
			//批次寫入WML02_LOG(檢核公式錯誤)(區分檔案上傳/線上編輯)
			//104.02.11 A12沒有檢核公式Utility.InsertWML02_LOG(m_year,m_month,user_id,user_name,report_no,input_method,filename,bank_codeList);
			...
			for(int i=0;i<bank_codeList.size();i++){
				...
				//=================================================================================
				Utility.printLogTime("A12-3 time");	
				//clear WML03(檢核其他錯誤)
				...
				*/								
				nonZero = false;
				dbData = Utility.getCountZero(m_year,m_month,(String)bank_codeList.get(i),report_no,"(baddebt_amt >0 or loss_amt>0 or profit_amt>0)");
				...
				*/
				Utility.printLogTime("A12-5 time");	
				...
				*/
				Utility.printLogTime("A12-6 time");	
			...
			Utility.printLogTime("A12-7 time");		
			if(input_method.equals("F")){//檔案上傳
			...
			if(!updateOK){
				//parserResult=false;
				errMsg = errMsg + "UpdateA12.doParserReport_A12 UpdateDB Error:"+DBManager.getErrMsg()+"<br>";
				System.out.println(DBManager.getErrMsg());
			}
			if(input_method.equals("W"))/*線上編輯*/ errMsg = upd_code +":" + errMsg;//104.02.11
			System.out.println("errMsg="+errMsg);
			Utility.printLogTime("A12-8 time");	
			if(input_method.equals("F")){//檔案上傳
			...
			Utility.printLogTime("A12-9 time");	
		}catch (Exception e) {
			//parserResult=false;
			errMsg = errMsg + "UpdateA12.doParserReport_A12 Error:"+e.getMessage()+"<br>";
			System.out.println("UpdateA12.doParserReport_A12="+e.getMessage());
		}
		return errMsg;
		//return parserResult;
	}
	...
	private static List getA12FileData(String report_no,String filename,String m_year,String m_month){
			String	txtline	 = null;			
			...
			List dbData = null;			
			String tmpAmt="";
			String baddebt_amt_cnt="";
			String loss_amt_cnt="";
			String profit_amt_cnt="";
			...
			try {
				...
				while ((txtline = in.readLine()) != null) {
						if (txtline.trim().equals("")){
							...
							detail.add("000000");//acc_code(A12無acc_code)
							...
							profit_amt_cnt=tmpAmt;
							//=====================================================================							
							detail.add(baddebt_amt_cnt);//轉銷呆帳金額-本月減少備抵呆帳(axx_tmp.amt)
							detail.add(loss_amt_cnt);//轉銷呆帳金額-本月直接認列損失(axx_tmp.amt1)
							detail.add(profit_amt_cnt);//存款準備率降低所增加盈餘(本月增加金額)(axx_tmp.amt2)
							detail.add("0");//amt3
							detail.add("0");//amt4
							detail.add("0");//amt5
							detail.add("0");//amt6
				...
				errMsg = errMsg + "UpdateA12.getA12FileData Error:"+e.getMessage()+"<br>";
			}
