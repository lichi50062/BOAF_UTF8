...
public class UpdateM01 {
	private static String errMsg = "";
	public String getErrMsg(){
		return errMsg;  
	}
	public static synchronized String doParserReport_M01(String report_no, String m_year,
		...
		List M01List = new LinkedList();//M01細部資料		
		...
		try { 
			...
			if(input_method.equals("F")){//檔案上傳
				...
				for(int j=0;j<M01List.size();j++){		
					String dataRange=(String)((List)M01List.get(j)).get(1);
					if(!dataRange.equals(sm_year+"MM") && 
					   ...
					   !dataRange.equals(sm_year+"ET")
						...
						dataList.add("資料期間[" + (String)((List)M01List.get(j)).get(1) +"]不存在");
						dataList.add(user_id);
						dataList.add(user_name);
				...
				if(errCount==0){	//資料期間無錯時才insert資料至DB中
			    	...
			    	if(dbData.size() == 0){//M01無資料時,才insert一筆Zero的資料
			    		System.out.println("InsertZeroM01");
			    		InsertZeroM01(m_year,m_month,bank_code);
			    	}else{//end of M01不存在時
			    		updateDBDataList = new ArrayList();	
						updateDBSqlList = new ArrayList();
						sqlCmd.delete(0,sqlCmd.length());
					...
					updateDBSqlList = new ArrayList();
					sqlCmd.delete(0,sqlCmd.length());
					sqlCmd.append("UPDATE M01 SET guarantee_cnt =?,");
					...
					sqlCmd.append(" WHERE m_year=? AND m_month=?");
					sqlCmd.append("   AND guarantee_item_no=?");
					sqlCmd.append("   AND data_range=?");
			    	for(int j=0;j<M01List.size();j++){//把上傳檔案裡的資料更新至M01				
			    		String dataRange=(String)((List)M01List.get(j)).get(1);
			    		...
			    		if( Long.parseLong((String)((List)M01List.get(j)).get(2)) > 0
			    		 || Long.parseLong((String)((List)M01List.get(j)).get(3)) > 0
						 || Long.parseLong((String)((List)M01List.get(j)).get(4)) > 0
						 || Long.parseLong((String)((List)M01List.get(j)).get(5)) > 0
						 || Long.parseLong((String)((List)M01List.get(j)).get(6)) > 0
						 || Long.parseLong((String)((List)M01List.get(j)).get(7)) > 0
						 || Long.parseLong((String)((List)M01List.get(j)).get(8)) > 0
						 || Long.parseLong((String)((List)M01List.get(j)).get(9)) > 0
						 || Long.parseLong((String)((List)M01List.get(j)).get(10)) > 0
						 || Long.parseLong((String)((List)M01List.get(j)).get(11)) > 0
						 || Long.parseLong((String)((List)M01List.get(j)).get(12)) > 0
						 || Long.parseLong((String)((List)M01List.get(j)).get(13)) > 0
						 || Long.parseLong((String)((List)M01List.get(j)).get(14)) > 0)
			    		{
			    		   nonZero = true;
			    		}
						if(nonZero){
							dataList =  new ArrayList();
							dataList.add((String)((List)M01List.get(j)).get( 2));
							dataList.add((String)((List)M01List.get(j)).get( 3));
							dataList.add((String)((List)M01List.get(j)).get( 4));
							dataList.add((String)((List)M01List.get(j)).get( 5));
							dataList.add((String)((List)M01List.get(j)).get( 6));
							dataList.add((String)((List)M01List.get(j)).get( 7));
							dataList.add((String)((List)M01List.get(j)).get( 8));
							dataList.add((String)((List)M01List.get(j)).get( 9));
							dataList.add((String)((List)M01List.get(j)).get(10));
							dataList.add((String)((List)M01List.get(j)).get(11));
							dataList.add((String)((List)M01List.get(j)).get(12));
							dataList.add((String)((List)M01List.get(j)).get(13));
							dataList.add((String)((List)M01List.get(j)).get(14));
							dataList.add(String.valueOf(Integer.parseInt(m_year)));
							dataList.add(String.valueOf(Integer.parseInt(m_month)));
							dataList.add((String)((List)M01List.get(j)).get(0));
			...
			if(nonZero && errCount == 0){//無其他錯誤時,才檢核合計金額
				sqlCmd.delete(0,sqlCmd.length());
				paramList = new ArrayList();
				sqlCmd.append(" select decode(guarantee_item_no,'0','0','1') as \"type\", ");
                ...
                sqlCmd.append(" from m01 a,m00_data_range_item b ");
                sqlCmd.append(" where m_year=? and m_month=?");
                sqlCmd.append("  and substr(a.data_range,4,2)=b.data_range ");
                sqlCmd.append("  and b.report_no='M01' ");
                sqlCmd.append(" group by decode(guarantee_item_no,'0','0','1'),a.data_range,b.input_order,b.data_range_name || '加總與總計不合' ");
                sqlCmd.append(" order by decode(guarantee_item_no,'0','0','1') desc,b.input_order,b.data_range_name || '加總與總計不合' ");
                paramList.add(String.valueOf(Integer.parseInt(m_year)));
                paramList.add(String.valueOf(Integer.parseInt(m_month)));
                dbData = DBManager.QueryDB_SQLParam(sqlCmd.toString(),paramList,"m_year,m_month,guarantee_cnt,loan_amt,guarantee_amt,loan_bal,guarantee_bal,over_notpush_cnt,over_notpush_bal,over_okpush_cnt,over_okpush_bal,repay_tot_cnt,repay_tot_amt,repay_bal_cnt,repay_bal_amt");
                
                updateDBDataList = new ArrayList();//99.11.17	
				for(int j=0;j<3;j++){
			...
			if(input_method.equals("F")){//檔案上傳
			   ...
			   //99.11.16updateDBSqlList.add("DELETE FROM WML01_UPLOAD where filename='"+filename+"'");
			...
			if(!updateOK){
				//parserResult=false;
				errMsg = errMsg + "UpdateM01.doParserReport_M01 UpdateDB Error:"+DBManager.getErrMsg()+"<br>";
			...
			errMsg = errMsg + "UpdateM01.doParserReport_M01 Error:"+e.getMessage()+"<br>";
			System.out.println("UpdateM01.doParserReport_M01="+e.getMessage());			
		}
		return errMsg;	
	}

	//讀取上傳檔案的資料
	private static List getM01FileData(String report_no,String filename){
			String	txtline	 = null;			
			List M01List = new LinkedList();
			List detail = null;
			List dbData = null;			
			...
			try {
				...
				errMsg = errMsg + "UpdateM01.getM01FileData Error:"+e.getMessage()+"<br>";
			}
			return M01List;
	}
	...
	private static boolean InsertZeroM01(String m_year,String m_month,String bank_code){
		List paramList = new ArrayList();
		StringBuffer sqlCmd = new StringBuffer();	
		...
		try{
			sqlCmd.append("select a.guarantee_item_no,b.data_range ");
			sqlCmd.append("from m00_guarantee_item a,m00_data_range_item b ");
			sqlCmd.append("where ((a.guarantee_item_no<>'0' and b.data_range_type<>'T') or ");
			sqlCmd.append("       (a.guarantee_item_no='0' and b.data_range_type='T') ");
			sqlCmd.append("      )  ");
			sqlCmd.append("  and b.report_no='M01' ");
			sqlCmd.append("order by a.input_order,b.input_order ");
			
			List data_div01 = DBManager.QueryDB_SQLParam(sqlCmd.toString(),paramList,"update_date");//資負表 	    
			sqlCmd.delete(0, sqlCmd.length());
			sqlCmd.append("INSERT INTO M01 ( M_YEAR,M_MONTH,GUARANTEE_ITEM_NO,DATA_RANGE ) VALUES (?,?,?,?)");
			
			for(int d1=0;d1<data_div01.size();d1++){
				String sm_year="";
				sm_year="0000"+m_year; 
				sm_year=sm_year.substring(sm_year.length()-3);
				dataList = new LinkedList();//儲存參數的data
				dataList.add(String.valueOf(Integer.parseInt(m_year)));
				dataList.add(String.valueOf(Integer.parseInt(m_month)));
				dataList.add((String)((DataObject)data_div01.get(d1)).getValue("guarantee_item_no"));
			...
			updateOK=DBManager.updateDB_ps(updateDBList);
			System.out.println("M01 Insert Zero OK??"+updateOK);
			if(!updateOK){
				errMsg = errMsg + "M01 Insert Zero Fail:"+DBManager.getErrMsg()+"<br>";
				System.out.println(DBManager.getErrMsg());
			}
		}catch(Exception e){
			errMsg = errMsg + "UpdateM01.InsertZeroM01 Error:"+e.getMessage()+"<br>";
			System.out.println("UpdateM01.InsertZeroM01 Error:"+e.getMessage());
		}
    	return updateOK;
	}
}
