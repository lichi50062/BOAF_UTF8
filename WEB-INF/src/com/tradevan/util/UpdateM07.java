...
public class UpdateM07{
	private static String errMsg = "";
	
	public String getErrMsg(){
		return errMsg;  
	}
	public static synchronized String doParserReport_M07(String report_no, String m_year,
		...
		List M07List = new LinkedList();//M07細部資料
		...
		try { 
			if(input_method.equals("F")){//若為檔案上傳,先將檔案讀取出來
				M07List = getM07FileData(report_no,filename);
				...
				System.out.println("M07List.size="+M07List.size());
			}else{
				user_id = szuser_id;
			...
			if(input_method.equals("F")){//檔案上傳
				...
				if(errCount==0){	//資料期間無錯時才insert資料至DB中
			    	...
			    	if(dbData.size() == 0){	//M07無資料時,才insert一筆Zero的資料
			    		System.out.println("InsertZeroM07");
			    		InsertZeroM07(m_year,m_month,bank_code);
			    	}else{//end of M07不存在時
			    		updateDBDataList = new ArrayList();	
						updateDBSqlList = new ArrayList();
						sqlCmd.delete(0,sqlCmd.length());
						sqlCmd.append(" INSERT INTO M07_LOG ");
					...
					updateDBSqlList = new ArrayList();
					sqlCmd.delete(0,sqlCmd.length());
					sqlCmd.append("UPDATE M07 SET guarantee_no_month   =?,");
			    	...
			    	for(int j=0;j<M07List.size();j++){//把上傳檔案裡的資料更新至M07
			    		//94.04.24 add 檢核上傳檔案內容的金額是否都為"0"
			    		if( Long.parseLong((String)((List)M07List.get(j)).get(1)) > 0
					     || Long.parseLong((String)((List)M07List.get(j)).get(2)) > 0
					     || Long.parseLong((String)((List)M07List.get(j)).get(3)) > 0					      
		    			 ||	Long.parseLong((String)((List)M07List.get(j)).get(4)) > 0
						 || Long.parseLong((String)((List)M07List.get(j)).get(5)) > 0
						 || Long.parseLong((String)((List)M07List.get(j)).get(6)) > 0
						 || Long.parseLong((String)((List)M07List.get(j)).get(7)) > 0
						 || Long.parseLong((String)((List)M07List.get(j)).get(8)) > 0
						 || Long.parseLong((String)((List)M07List.get(j)).get(9)) > 0
						 || Long.parseLong((String)((List)M07List.get(j)).get(10)) > 0
			    		 || Long.parseLong((String)((List)M07List.get(j)).get(11)) > 0
			    		 || Long.parseLong((String)((List)M07List.get(j)).get(12)) > 0
			    		 || Long.parseLong((String)((List)M07List.get(j)).get(13)) > 0)
			    		{
			    		   nonZero = true;
			    		}
			    		...
			    		if(nonZero){
							dataList =  new ArrayList();
							dataList.add((String)((List)M07List.get(j)).get( 1));     
							dataList.add((String)((List)M07List.get(j)).get( 2));
							dataList.add((String)((List)M07List.get(j)).get( 3));
							dataList.add((String)((List)M07List.get(j)).get( 4));
							dataList.add((String)((List)M07List.get(j)).get( 5));
							dataList.add((String)((List)M07List.get(j)).get( 6));
							dataList.add((String)((List)M07List.get(j)).get( 7));
							dataList.add((String)((List)M07List.get(j)).get( 8));
							dataList.add((String)((List)M07List.get(j)).get( 9));
							dataList.add((String)((List)M07List.get(j)).get(10));
							dataList.add((String)((List)M07List.get(j)).get(11));
							dataList.add((String)((List)M07List.get(j)).get(12));
							dataList.add((String)((List)M07List.get(j)).get(13));
							dataList.add(String.valueOf(Integer.parseInt(m_year)));
							dataList.add(String.valueOf(Integer.parseInt(m_month)));
							dataList.add((String)((List)M07List.get(j)).get(0));						
							updateDBDataList.add(dataList);
			    		}//end of nonZero
			    	}//end of for
					...
					updateDBSqlList = new ArrayList();
					sqlCmd.delete(0,sqlCmd.length());
			    	sqlCmd.append("UPDATE M07 ");
					sqlCmd.append("SET guarantee_no_month  =(select sum(guarantee_no_month)   from M07 where m_year=? and m_month=? and area_no not in ('0','1','2','A','E')), ");
					sqlCmd.append("    guarantee_amt_month =(select sum(guarantee_amt_month)  from M07 where m_year=? and m_month=? and area_no not in ('0','1','2','A','E')), ");
					sqlCmd.append("    loan_amt_month      =(select sum(loan_amt_month)       from M07 where m_year=? and m_month=? and area_no not in ('0','1','2','A','E')), ");
					sqlCmd.append("    guarantee_no_year   =(select sum(guarantee_no_year)    from M07 where m_year=? and m_month=? and area_no not in ('0','1','2','A','E')), ");
					sqlCmd.append("    guarantee_amt_year  =(select sum(guarantee_amt_year)   from M07 where m_year=? and m_month=? and area_no not in ('0','1','2','A','E')), ");
					sqlCmd.append("    loan_amt_year       =(select sum(loan_amt_year)        from M07 where m_year=? and m_month=? and area_no not in ('0','1','2','A','E')), ");
					sqlCmd.append("    guarantee_no_totacc =(select sum(guarantee_no_totacc)  from M07 where m_year=? and m_month=? and area_no not in ('0','1','2','A','E')), ");
					sqlCmd.append("    guarantee_amt_totacc=(select sum(guarantee_amt_totacc) from M07 where m_year=? and m_month=? and area_no not in ('0','1','2','A','E')), ");
					sqlCmd.append("    loan_amt_totacc     =(select sum(loan_amt_totacc)      from M07 where m_year=? and m_month=? and area_no not in ('0','1','2','A','E')), ");
					sqlCmd.append("    guarantee_bal_no    =(select sum(guarantee_bal_no)     from M07 where m_year=? and m_month=? and area_no not in ('0','1','2','A','E')), ");
					sqlCmd.append("    guarantee_bal_amt   =(select sum(guarantee_bal_amt)    from M07 where m_year=? and m_month=? and area_no not in ('0','1','2','A','E')), ");
					sqlCmd.append("    guarantee_bal_p     =(select sum(guarantee_bal_p)      from M07 where m_year=? and m_month=? and area_no not in ('0','1','2','A','E')), ");
					sqlCmd.append("    loan_bal            =(select sum(loan_bal)             from M07 where m_year=? and m_month=? and area_no not in ('0','1','2','A','E')) ");
					sqlCmd.append("where m_year=? AND m_month=?");
			    	...
			    	System.out.println("M07 update data OK??"+updateOK);			    	
			    }
			}//end of if 檔案上傳	
			...
			if(nonZero && errCount == 0){//無其他錯誤時,才檢核合計金額
                ...
                sqlCmd.append("order by decode(area_no,'0','0','1')");
                dbData = DBManager.QueryDB_SQLParam(sqlCmd.toString(),paramList,"m_year,m_month,guarantee_no_month,guarantee_amt_month,loan_amt_month,guarantee_no_year,guarantee_amt_year,loan_amt_year,guarantee_no_totacc,guarantee_amt_totacc,loan_amt_totacc,guarantee_bal_no,guarantee_bal_amt,guarantee_bal_p,loan_bal");		
			...
			if(!updateOK){
				//parserResult=false;
				errMsg = errMsg + "UpdateM07.doParserReport_M07 UpdateDB Error:"+DBManager.getErrMsg()+"<br>";
			...
			//parserResult=false;
			errMsg = errMsg + "UpdateM07.doParserReport_M07 Error:"+e.getMessage()+"<br>";
			System.out.println("UpdateM07.doParserReport_M07="+e.getMessage());			
		}
	...
	//讀取上傳檔案的資料
	private static List getM07FileData(String report_no,String filename){
			String	txtline	 = null;			
			List M07List = new LinkedList();
			List detail = null;
			List dbData = null;	
			...
			try {
				...
				doLoop://將txt檔案儲存至LinkedList - M07List
				...
				errMsg = errMsg + "UpdateM07.getM07FileData Error:"+e.getMessage()+"<br>";
			}
	...
	private static boolean InsertZeroM07(String m_year,String m_month,String bank_code){
		List paramList = new ArrayList();
		StringBuffer sqlCmd = new StringBuffer();	
		...
		try{
    		...
    		sqlCmd.delete(0, sqlCmd.length());
			sqlCmd.append("INSERT INTO M07 ( M_YEAR,M_MONTH,AREA_NO ) VALUES (?,?,?)");
			...
			updateOK=DBManager.updateDB_ps(updateDBList);
			System.out.println("M07 Insert Zero OK??"+updateOK);
			if(!updateOK){
				errMsg = errMsg + "M07 Insert Zero Fail:"+DBManager.getErrMsg()+"<br>";
				System.out.println(DBManager.getErrMsg());
			}
		}catch(Exception e){
			errMsg = errMsg + "UpdateM07.InsertZeroM07 Error:"+e.getMessage()+"<br>";
			System.out.println("UpdateM07.InsertZeroM07 Error:"+e.getMessage());
		}
